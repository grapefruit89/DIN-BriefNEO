/**
 * js/cma-sensor.js — v4.0 CMA-Sensor (SPEC-031)
...
export class CMASensor {
  ...
  init() {
    if (!this.sensor || !this.root) {
      console.warn(
        "âš ï¸ [CMA-Sensor] Required elements <din-cma-sensor> or <din-page-wrapper> not found.",
      );
      return;
    }
    ...
    this.observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          if (isOverflow) {
            console.warn(
              "âš ï¸ [CMA-Sensor] Layout Violation: Content exceeds page 1 threshold.",
            );
          }
        });
      },
