  ...
  try {
    ...
    const after = await page.evaluate(() => {
      ...
      return {
        layout: document.body.dataset.layout,
        addrTop: window.getComputedStyle(addr).top,
        fold1Top: window.getComputedStyle(document.querySelector('.mark-fold-1')).top
      };
    });
    console.log('Nach Klick (Soll Form A):', after);

    if (after.layout === 'form-a' && after.addrTop.includes('102')) { // 27mm * 3.78 ≈ 102px
        console.log('✅ SUCCESS: Layout-Switch funktioniert technisch!');
    } else {
        console.error('❌ FAILURE: Layout-Switch hat keine Auswirkung im DOM/CSS.');
    }
