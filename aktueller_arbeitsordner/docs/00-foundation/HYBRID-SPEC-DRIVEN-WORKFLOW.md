...
> [!INFO] Leitplanken (`_9` im Dezimalrahmen)
> Dieser Workflow ist verbindlich für alle Entwicklungs-Sessions. Er gehört inhaltlich zum Fundament — unveränderbar ohne expliziten menschlichen Entscheid. Verwandte Governance-Dokumente: [[constitution]] | [[Immutable-Law-Catalog]] | [[ADR-ANTIPATTERN]]
...
- **Light Mode** (Standard für die meisten Änderungen): Nur die Kernschritte Pre-Build → Änderung → Post-Build (100%) → Loggen + kurzer Generalisierungs-Vermerk im [[DECISION-LOG]]. Kein zwingendes [[spec]].
...
1. **Constitution** (setup / major updates)
   ...
   - Location: `.specify/constitution.md` (or reference to `constitution.md` + `Immutable-Law-Catalog.md`).
...
5. **Implement**
   ...
   - Strictly follow constitution, [[Immutable-Law-Catalog]], AGENTS.md, and the spec (if Full Mode).
...
7. **Generalize & Extract** (Testballon duty)
   ...
   - Full Mode: Ausführlich im [[spec]] + Vorschläge.

   - Propose concrete pull/extraction steps.

   - Update MIGRATION-ROADMAP-TO-BOILERPLATE.md or [[DECISION-LOG]].
...
- `constitution.md` / [[Immutable-Law-Catalog]] : Core philosophy – parts can be generalized.
