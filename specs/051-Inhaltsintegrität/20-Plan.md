...
# 20 — Plan: Technische Realisierung (Integrity)
...
- `EditContext` liefert Roh-Text ohne HTML-Störsignale.
...
- **Target:** `<din-text-mirror>` (Positioned via CSS Anchor/Absolute über Source).
- **Injektion:** `mirror.setHTML(html, { sanitizer: PLATINUM_SANITIZER })`.
...
- **CSS Custom Highlight API:** Optional für Syntax-Highlighting im Edit-Mode.
