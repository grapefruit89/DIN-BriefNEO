...
// build/ ist bewusst nicht versioniert (.gitignore) — bei einem frischen Checkout
// existiert es also nicht. Das Skript legt sein eigenes Ausgabeverzeichnis an,
// statt vorauszusetzen, dass es schon da ist.
fs.mkdirSync(path.join(targetDir, path.dirname(outputSqlFile)), { recursive: true });
...
function main() {
  ...
  if (!report.success || report.score < 100) {
    console.error('🔴 Build failed: Critical architectural violations found or Fitness Score < 100%! Generation stopped.');
    process.exit(1);
  }
...
`;
    ...
    fs.writeFileSync(path.join(targetDir, 'docs', '10-architecture', 'Code-Referenzen.md'), md, 'utf8');
  }
