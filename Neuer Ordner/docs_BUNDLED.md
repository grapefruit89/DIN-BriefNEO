---
title: "Antigravity Context Bundler – docs"
created: 2026-03-27
exported_at: "2026-03-27 15:41:20"
source_path: "C:\Users\morit\Desktop\DIN-BriefNEO\docs"
file_count: 4
total_chars: 386089
total_lines: 8851
version: "4.1"
...
# Antigravity Context Bundler – docs
...
### Top-Keywords
`chrome` · `alle` · `constraint` · `din-element` · `custom-tag` · `matrix` · `zone` · `standard` · `zeile` · `greetings` · `gesteuert` · `dokument` · `blink-direct` · `native` · `engine` · `binding` · `ssot` · `locked` · `name` · `check` · `test` · `desc` · `true` · `const` · `globalthis` · `console` · `navigator` · `din-briefneo` · `supports` · `item`
...
|---|------|-----|-------|--------|---------|----------|
| 1 | `DIN_5008_SPEC.md` | .md | 3.71 KB | 65 | 3716 | constraint, din-element, custom-tag, matrix |
| 2 | `SYSTEM_PLATINUM_SSOT.md` | .md | 3.92 KB | 71 | 3922 | constraint, din-element, custom-tag, standard |
| 3 | `audit\SUPER_AUDIT.md` | .md | 6.03 KB | 98 | 5909 | name, check, test, desc |
| 4 | `PLATINUM_HANDBOOK.md` | .md | 372.33 KB | 8617 | 372542 | must, html, kein, status |
...
| Feld | Wert |
|------|------|
| Pfad | `DIN_5008_SPEC.md` |
| Typ | `.md` |
| Größe | 3.71 KB |
| Zeilen | 65 |
| Zeichen | 3716 |
| Keywords | `constraint` `din-element` `custom-tag` `matrix` `zone` `standard` `alle` `zeile` `greetings` `gesteuert` `chrome` `dokument` `blink-direct` `engine` `binding` |
...
| Feld | Wert |
|------|------|
| Pfad | `SYSTEM_PLATINUM_SSOT.md` |
| Typ | `.md` |
| Größe | 3.92 KB |
| Zeilen | 71 |
| Zeichen | 3922 |
| Keywords | `constraint` `din-element` `custom-tag` `standard` `matrix` `zone` `alle` `zeile` `greetings` `gesteuert` `ssot` `locked` `chrome` `dokument` `blink-direct` |

```markdown
# 📜 SYSTEM_Standard_SSOT.md
# 💎 Standard_PROTOCOL_V3.LOCKED
...
**Gezeichnet:**
*Der Standard Architect (Gemini CLI)*
```

---

### [[audit/SUPER_AUDIT.md]]

| Feld | Wert |
|------|------|
| Pfad | `audit\SUPER_AUDIT.md` |
| Typ | `.md` |
| Größe | 6.03 KB |
| Zeilen | 98 |
| Zeichen | 5909 |
| Keywords | `name` `check` `test` `desc` `true` `const` `globalthis` `native` `console` `navigator` `din-briefneo` `supports` `item` `chrome` `color` |
...
| Feld | Wert |
|------|------|
| Pfad | `PLATINUM_HANDBOOK.md` |
| Typ | `.md` |
| Größe | 372.33 KB |
| Zeilen | 8617 |
| Zeichen | 372542 |
| Keywords | `must` `html` `kein` `status` `mandatory` `chrome` `system` `spec-` `native` `core` `logic` `alle` `adr-` `muss` `paper` |

```markdown
# 💎 Standard DEVELOPER HANDBOOK (OMNIBUS EDITION)
# Stand: März 2026 | Status: EXHAUSTIVE COMPILATION

> **DOKTRIN-NOTIZ:** Dieses Handbuch ist absichtlich *nicht* zusammengefasst. Es enthält **alle** Spezifikationen, Architektur-Doktrinen, Pläne, Protokolle und Verfassungsdokumente in ihrer vollständigen, ungekürzten Originalfassung. Es dient als ultimative, verlustfreie Single Source of Truth (SSoT) für die GitHub-Publikation.
...
## � Dokument: 00_REQUIREMENTS_MASTER_v1.0.0.md (Quelle: .brain)

# ?? DIN-BriefNEO MASTER-MONOLITH (v1.0.0)
## STATUS: ZEMENTIERT | DOKTRIN: Core GRADE Standard

### ?? DIE VERFASSUNG (CHROME 147+ BASELINE)
...
2. **NO-JS UI DOKTRIN:** Imperatives JS f�r UI-Effekte ist illegal. Tooltips, Popovers und Dialoge nutzen exklusiv **CSS Anchor Positioning** (`anchor-name`, `position-anchor`) und native **Invoker Commands** (`commandfor`).
3. **SCROLL-LOGIK:** JS-Scroll-Listener sind durch native CSS `@container scroll-state()` ersetzt. Die �berlauf-Erkennung findet auf Render-Ebene statt.
...
5. **PLAINTEXT-VERSIEGELUNG:** `contenteditable="true"` ist verboten. Nutzung der **EditContext API** in Kombination mit **CSS Custom Highlights** f�r den Ghost-Mirror (Core Grade Isolation).
...
tags: [layout, din-5008, Core-grade]
status: cemented
weight: 100
...
- **Footer**: 269mm.
- **Verification**: Diese Werte wurden gegen MehrCurry/briefversand abgeglichen und sind ab sofort als "Core Grade" zementiert.
...
**Why this priority**: Das Herzstck der "Core-Grade" Wartbarkeit.
...
- **Exclusive Toggle**: Durch Nutzung des `name`-Attributs (z.B. `<details name="settings">`) MUST sicherstellen, dass sich andere Bereiche automatisch schlieen (Core-Grade UI ohne JS).
...
---
id: SPEC-037
title: DIN-Logic Formatter (Core Grade)
tags: [specification, din-5008, platin]
status: cemented
weight: 90
criticality: HIGH
created: 2026-03-20
---
# Feature Specification: DIN-Logic Formatter (Core Grade)
...
- **Rationale**: Ein Standard-Briefgenerator muss sicherstellen, dass Zahlen und Daten den hchsten professionellen Standards entsprechen.
...
---
id: SPEC-038
title: Core-Grade Intelligence Suite (Superpowers)
tags: [specification, din-5008, platin]
status: cemented
weight: 95
criticality: HIGH
created: 2026-03-20
---
# Feature Specification: Core-Grade Intelligence Suite (Superpowers)
...
- **SC-002**: **Core Precision**: Die Paginierung "X von Y" ist auf jeder Seite (inklusive Seite 1, falls > 1) in Echtzeit korrekt berechnet.
...
- **Lexicon Check**: "Toast-ID", "Compliance-Trigger", "UI-Hint", "Core-Alert".
...
- **SC-002**: **Core Accuracy**: Die Umschaltung landet auf beiden Seiten exakt auf den Mikrometer-Werten der SVG-Referenz (27mm vs 45mm).
...
- **Rationale**: Ein "Standard" Brief-Generator muss auch die Anforderungen an Geschftsbriefe ( 80 AktG,  35a GmbHG) perfekt abbilden.
...
- **FR-008: Private Key Model**: Das System nutzt das "Core-Grade Privacy Model": Der Nutzer trgt seine *eigene* Client-ID und Secret in den NEO-Einstellungen ein. NEO agiert nur als lokaler Vermittler (keine zentrale Registrierung durch den Entwickler ntig).
...
# Feature Specification: Core Command Center (Keyboard & Shortcuts)
...
tags: [typography, printing, Core-grade]
status: cemented
weight: 85
criticality: HIGH
created: 2026-03-20
...
---
id: SPEC-052
title: Core Signal & Event Architecture
...
created: 2026-03-20
---
# Feature Specification: Core Signal Architecture
...
created: 2026-03-20
---
# Feature Specification: Core Accessibility
...
- **Logik**: Der Fokus-Ring (Visual Focus) MUST im Expert-Mode (SPEC-047) und Normal-Mode deutlich sichtbar sein (Core-Blue).
...
tags: [security, hardening, Core-grade]
status: cemented
weight: 100
criticality: CRITICAL
created: 2026-03-20
...
- **Logik**: `script-src 'self'` und `style-src 'self'`. Externe Skripte sind STRENG VERBOTEN (Core Standard).
...
---
id: SPEC-055
title: Core Typography Hardening
...
created: 2026-03-20
---
# Feature Specification: Core Typography Hardening
...
Einf�hrung der Sensoren zur Messung physischer DIN-Verste.
...
## File: plans/065-Standard-integrity/plan.md
---
id: PLAN-065
spec: SPEC-053
title: Core Grade Standard Integrity � EditContext & A11y
status: draft
anti-patterns: [ANTI-023, ANTI-025]
adr: [ADR-008, ADR-011]
---

# Implementation Plan: PLAN-065 � Standard Integrity
...
**Target Platform**: Chrome 147+ (Core Grade Standard)
...
#paper {
  ...
  overflow: hidden; /* Core Grade: kein Scrollen am Blatt */
}
~~~
...
## 1. Constitution Check (Core Grade)
| Gate | Status | Notiz |
|---|---|---|
...
- **Core Grade**: Die physische Wahrheit liegt im HTML-Attribut (SSoT).
...
## 4. Constraint Check
Der Akinator darf den Brief **niemals eigenmchtig ndern**. Er gibt nur Empfehlungen aus (Core Grade: Der Pilot/Nutzer behlt die Kontrolle).

---

## File: plans/061-Standard-pipeline/plan.md
---
id: PLAN-061
spec: SPEC-061
title: Implementation of the Standard Validation Pipeline (PVP)
status: cemented
anti-patterns: [ANTI-023, ANTI-025]
adr: [ADR-003, ADR-008]
---

# PLAN-061: Standard Validation Pipeline (PVP)
...
Die Datei `scripts/post-session-verify.ps1` wird um die Sektion `Standard MANDATE CHECK` erweitert.
...
## Core Grade Pflichtfelder (Validation Logic)
...
## � Dokument: 00_source_inventory_v1.0.0.md (Quelle: .brain)

---
tags: [Core-grade, Standard-2026, source-mapping, inventory]
status: cemented
version: 1.0.0
...
Diese Tabelle gibt eine �bersicht �ber alle Dokumente im Ordner `.\quellen` und deren architektonischen Nutzwert f�r das Projekt DIN-BriefNEO.

| Dateiname | Typ | Inhaltlicher Fokus | Arch. Wert | Notiz |
|:---|:---|:---|:---|:---|
| Briefvorlage.html | HTML | Interaktive Briefvorlage | Low | Nutzt Bootstrap & JS-Hacks (Legacy). Gut f�r Feldnamen. |
| Gemini-Legacy...md | MD | Migration-Spec | High | Basis f�r SDD-Workflow und IMR-Logik. |
| latex.pdf | PDF | LaTeX Dokumentation | Medium | Inspiration f�r `latex.base` Layer (Typografie). |
| readme (1).md | MD | CSS Framework Liste | Medium | �bersicht �ber Pico/Beer CSS (Extract Candidates). |
| HTML_&_CSS_f�r_Dummies_... | PDF | Grundlagen | Low | Standardwissen, wenig f�r Core-Grade Spezialisierung. |
| Wolf, J�rgen - HTML5... | EPUB | Handbuch | Medium | Umfassendes Nachschlagewerk f�r native Web-APIs. |
| Peter M�ller - Einstieg... | PDF | Responsive Design | Medium | Barrierefreiheit & Grid-Grundlagen. |
| Beer CSS | Info | UI Framework | High | Vorlage f�r `ui.theme` (Material Design Extract). |
| Pico.css | Info | Classless CSS | High | Vorlage f�r `ui.theme` (Aria-Invalid/Dialog Extracts). |
...
## N�chste Schritte: Integration in Blueprint
- Extraktion von mathematischen `text-box-trim` Ans�tzen aus modernen CSS-Artikeln (falls vorhanden).
- �bernahme von `cap-height` basierten Alignment-Strategien in `03_technical_blueprint_v1.3.1.md`.


---


## � Dokument: 00_source_inventory_v1.1.0.md (Quelle: .brain)

---
tags: [Core-grade, Standard-2026, source-mapping, inventory, deep-scan]
status: cemented
version: 1.1.0
last_audit: 2026-03-23
id: BRAIN-000-INV
...
## I. DIE HEILIGEN F�NF (DEEP-SCAN KERNQUELLEN)
Die folgenden 5 Dokumente wurden bin�r und textuell tiefenanalysiert. Die extrahierten Fakten bilden das physikalische und architektonische Fundament des Projekts.
...
| `Wolf, J�rgen - HTML5 und CSS3...` | Native API Specs: `<dialog open>` mit `showModal()`, HTML5 Constraint Validation (`novalidate`, `pattern`), `inert`-Verhalten f�r modale Traps. | `20_specify_chrome147`, `04_physical_constants` |
| `Peter M�ller - Einstieg... Barrierefreie...`| A11Y Specs: `Skip-Link` (Sprungmarke `#top` mit `.visually-hidden`), `aria-live` f�r dynamische Status-Updates (Fehlermeldungen). | `03_technical_blueprint`, `20_specify_chrome147` |
...
Alle weiteren Dateien im Verzeichnis `.\quellen` wurden auf architektonische Hebelwirkung gepr�ft.
...
| `9783836290890 Einstieg in HTML und CSS...pdf` | CSS-Spezifit�t: Identifikation von ID-�berschreibungs-Risiken. | `05_anti_pattern_registry` |
...
| `Code Campus - HTML5 und CSS3 f�r Einsteiger.epub` | Formular-Validation: Native Pseudo-Klassen `:valid` und `:invalid` als JS-Ersatz. | `03_technical_blueprint` |
...
| `CSS Web Design for Dummies@englishbooks.pdf` | Kaskaden-Tiefen: Vermeidung von CSS-Regeln mit einer Spezifit�t > 0,2,0. | `05_anti_pattern_registry` |
| `desktop.ini` | [SYSTEM FILE - NO PAYLOAD] | - |
| `Einstieg in HTML und CSS (Peter Mueller).epub` | Relative Units: `rem` f�r UI, aber expliziter Ausschluss f�r DIN-Ma�e (nur `mm`). | `04_physical_constants` |
| `html xhtml css for dummies 7th edition.pdf` | Legacy-Tags: Verbot von veralteten Tags (`<font>`, `<center>`). | `05_anti_pattern_registry` |
| `html xhtml and css all-in-one for dummies...` | CSS-Print: `@media print { display: none; }` f�r Ghost-Marker. | `03_technical_blueprint` |
| `HTML_&_CSS_f�r_Dummies_Florence_Maurice.pdf` | CSS Flexbox/Grid: Fallback-Routinen f�r alte Browser (Ignoriert wg. Chrome 147+ Mandat). | `05_anti_pattern_registry` (TOMB-Check) |
| `HTML_5_und_CSS3_f�r_Einsteiger_Der_leichte...` | Media-Queries: Ablehnung exzessiver Breakpoints zugunsten von Fluid-Typography. | `03_technical_blueprint` |
...
| `Paul Fuchs HTML 5 und CSS3 fuer Einsteiger...pdf`| Box-Sizing: `box-sizing: border-box` als absoluter Standard f�r alle UI-Komponenten. | `03_technical_blueprint` |
...
| `Wolf, J�rgen - HTML5 und CSS 3.epub` | Audio/Video-Tags: (Irrelevant f�r DIN-Brief, aber Verbot von Auto-Play-Attributen). | `05_anti_pattern_registry` |


---


## � Dokument: 01_constitution_v2.0.0.md (Quelle: .brain)

---
tags: [Core-grade, Standard-2026, architecture, constitution]
status: cemented
version: 2.0.0
last_audit: 2026-03-23
id: BRAIN-001-CONST
...
### �1 Technologische Hierarchie (The Core Pyramid)
Jedes Feature MUSS auf der niedrigstm�glichen Ebene implementiert werden:
1. **Nativ HTML** (Struktur, Semantik, Interaktion via Popover/Commanders)
2. **Nativ CSS** (Layout via `@layer`, Pr�zision via `mm`, Logik via `:has()`)
3. **Vanilla JavaScript** (Datenhaltung/IMR, Ghost-Mirror Sync, LocalStorage)
4. **Public APIs** (Anonyme Dienste f�r PLZ/Zinsen � optional & resilient)

### �2 Das Mogel-Prinzip (Zero-Width-Marker)
Die physikalische 1:1 Druckvorschau ist heilig. 
- Im Editor sichtbare Markdown-Steuerzeichen (`*`, `_`) d�rfen den Textfluss NICHT durch physikalische Breite beeinflussen.
- L�sung: `md-marker { display: inline-block; width: 0; overflow: hidden; }` im Mirror-Layer.

### �3 Rigidity Clause (Zonierung)
Das Dokument wird in zwei unverr�ckbare Kategorien unterteilt:
- **Kategorie A (Starre Kopfdaten):** Alle `din-*` Tags au�er Body. Millimeter-pr�zise, einzeilig, `plaintext-only`.
...
## � Dokument: 01_CORE_SPEC_v1.2.0.md (Quelle: .brain)

# CORE-SPEC: SEMANTIC ELEMENTS & TEMPORAL ENGINE (v1.2.0)
# Status: CEMENTED | Doctrine: Chrome 147+ Baseline | Stand: M�rz 2026
...
Vollst�ndiger Ersatz des mutierbaren `Date`-Objekts durch die native Temporal API.
- **Engine:** `Temporal.Now.plainDateISO()` f�r die Erfassung.
- **Persistence:** Speicherung ausschlie�lich im ISO-8601 String-Format.
- **Formatting:** `Intl.DateTimeFormat('de-DE')` zur Render-Zeit.

## 3. PHYSICAL GEOMETRY (DIN 5008:2020)
- **Form A:** Header-H�he fixiert auf 27mm.
- **Form B:** Header-H�he fixiert auf 45mm.
- **Implementation:** Steuerung ausschlie�lich �ber CSS Custom Properties (Variables), die aus dem globalen State gespeist werden.
...
## � Dokument: 02_TECH_SPEC_v1.0.0.md (Quelle: .brain)

# TECH-SPEC: THE NO-JS DOCTRINE & NATIVE BASELINE (v1.0.0)
# Status: CEMENTED | Doctrine: Core Grade Standard | Stand: M�rz 2026

## I. OBERSTES GEBOT: LOGIC-LITE ARCHITEKTUR
JavaScript existiert ausschlie�lich als unsichtbare Fachlogik (Der Ghost). Der Browser (Chrome 147+ Engine) �bernimmt das Rendering und Layout nativ (Der Mirror). Jede Form von DOM-Manipulation f�r Layoutzwecke ist ein fataler Architekturfehler.
...
- **EditContext API:** Die Eingabe wird �ber die native EditContext API gepuffert. Der Browser �bergibt Events direkt an die Vanilla-JS-Datenmatrix, ohne das DOM eigenst�ndig zu ver�ndern.
- **Custom Highlight API:** Formatierungen (Markdown, Marker) werden ausschlie�lich �ber `::highlight()` projiziert. Der Text im DOM bleibt reiner, unformatierter Text (Isomorphie-Garantie).
...
- **Fragmentierungs-Schutz:** Zwingende Nutzung von `position-visibility: anchors-visible`. Overlays werden bei Seitenumbr�chen (Paged Media) nativ ausgeblendet, wenn ihr Anker nicht sichtbar ist.
...
- **Scroll-State Queries:** Status-Abfragen (wie Sticky-Header) erfolgen nativ �ber `@scroll-state(stuck: top)`. Scroll-Listener (TOMB-V002) sind verboten.
- **Elastic UI (Field-Sizing):** Die H�he von Textfeldern wird nativ durch `field-sizing: content` gesteuert. Intrinsic Guards (`max-height`) sch�tzen die Layout-Stabilit�t.
...
- **Anti-FOUC Boot:** Initiales UI-Rendering nutzt `@starting-style` f�r flackerfreie �berg�nge nach dem IMR-Load.
- **Salutation Engine:** Deterministische State-Machine im Vanilla-JS f�r die Briefanreden-Logik.
- **Smart Document Title:** Deterministische Generierung des Dateinamens/Titels via Temporal API aus dem Betreff-Feld.

---
**GEZ. LEAD SYSTEMS ARCHITECT**
"Core Grade bedeutet: Fehler m�ssen strukturell unm�glich sein."

---


## � Dokument: 02_TECH_SPEC_v1.1.0.md (Quelle: .brain)
...
- **Logic:** Der Browser �bergibt `textupdate` Events direkt an den JS-State. Das DOM dient nur als unstrukturierte Textschicht.
...
- **Visibility:** `position-visibility: anchors-visible` zur Vermeidung von Layout-Glitches bei Seitenumbr�chen.

## 4. INTRINSIC SIZING (field-sizing)
- **Engine:** `field-sizing: content` zur automatischen H�henanpassung von Eingabefeldern.
...
## � Dokument: 03_SAFE_SPEC_v1.0.0.md (Quelle: .brain)
...
- **Logic:** Nur der Tab, der den exklusiven Lock h�lt (Leader), darf aktiv in das OPFS schreiben. 
- **Heartbeat:** Der Lock wird f�r die gesamte Lebensdauer des Tabs gehalten. Stirbt der Tab (Crash/Close), wird der Lock vom Browser nativ freigegeben und ein Follower-Tab wird automatisch zum neuen Leader (Phoenix-Eigenschaft).
...
- **Access:** `createSyncAccessHandle({ mode: "readwrite-unsafe" })` f�r maximale Performance (C++ native Speed).
- **I/O Operations:**
    - `accessHandle.truncate(0)` vor jedem neuen Schreibvorgang.
    - `accessHandle.write(buffer)` f�r den atomaren Daten-Dump.
...
- **Integrity:** Ein `Temporal.Now.instant().epochMilliseconds` Zeitstempel im JSON-Header dient als Validit�ts-Check f�r das Journal.

## 4. SYSTEM-LEVEL IDLE DETECTION (Native Debounce)
- **Engine:** `IdleDetector API`.
- **Threshold:** 60.000ms (1 Minute Inaktivit�t).
- **Behavior:** Der `IOCoordinator` triggert den Save-Worker erst, wenn der Browser `userState: 'idle'` meldet. Dies schont CPU-Ressourcen w�hrend intensiver Schreibphasen.

---
**GEZ. LEAD SYSTEMS ARCHITECT**
"Core Grade: Synchronized, isolated, and hardware-flushed."

---


## � Dokument: 04_physical_constants_v1.0.0.md (Quelle: .brain)

---
tags: [Core-grade, Standard-2026, architecture, physical-constants, hardware-reference]
status: cemented
version: 1.0.0
last_audit: 2026-03-23
id: BRAIN-004-PHY
...
Dieses Dokument ist die **Hardware-Referenz** des DIN-BriefNEO. Es enth�lt ausschlie�lich nackte Zahlen, Regex-Muster und Millimeter-Werte. Jede Variable in CSS oder JS, die physikalische Berechnungen anstellt, muss sich aus dieser Quelle speisen.

## I. TYPOGRAFIE & GEOMETRIE (Aus `latex.pdf` / KOMA-Script)
- **Base Font-Size:** `14pt` (Skaliert von Standard 12pt f�r optimale Lesbarkeit)
...
- **Signature Vertical Space:** `5\baselineskip` (ca. `5em` Abstand zwischen Gru�formel und Name f�r die physische Unterschrift)
...
- `CMA_PAGE_OVERFLOW_TRIGGER`: `280.0mm` (Ausl�ser f�r Seite 2)

## III. NATIVE HTML5 CONSTRAINTS & REGEX (Aus Wolf, HTML5 & CSS3)
Browser-native Validierungs-Muster f�r das `pattern`-Attribut:
- **PLZ (Deutschland):** `pattern="[0-9]{5}"` (`maxlength="5"`)
- **Telefon:** `pattern="[\d\s\+\-\(\)]{7,20}"`
- **Datum (Deutsch):** `pattern="\d{2}\.\d{2}\.\d{4}"`
- **Native Types:** `type="email"`, `type="tel"`, `type="date"` (l�st Zero-JS Picker aus)
...
- **LocalStorage Quota Limit:** `~5MB` (Hard-Limit der Browser-Engines; Trigger f�r G-002 Overflow Protocol)

## V. ACCESSIBILITY (Aus Peter M�ller, Barrierefreie...)
- **Skip-to-Content ID:** `#top` (Das Anker-Ziel auf dem `<main>` oder `#paper` Wrapper)
- **Visually Hidden Class:** CSS-Regel f�r den Skip-Link (0x0 Pixel Dimension, aber fokussierbar f�r Tastatur-Nutzer)
- **Aria-Live Region:** `aria-live="polite"` (F�r Statusmeldungen des Akinators/Circuit Breakers ohne den Fokus zu stehlen)


---


## � Dokument: 05_anti_pattern_registry_v2.0.0.md (Quelle: .brain)

---
tags: [Core-grade, Standard-2026, spec-kit, anti-patterns, cemetery]
status: cemented
version: 2.0.0
last_audit: 2026-03-23
id: BRAIN-005-APR
...
## I. DIE OBERSTE REGEL: STRUKTURELLE PR�VENTION [MANDATE-PLN]
Jede technische Entscheidung wird gegen dieses Register validiert. Ein Core Grade System repariert keine Fehler � es macht sie unm�glich.
...
| TOMB-S003 | Code-First Entwicklung | SPEC -> Specify -> Plan -> Code | Core Grade Process |
...
## III. MANDATE-CHECK
Jede neue Spezifikation MUSS best�tigen: "Valide gegen Anti-Pattern Registry v2.0".


---


## � Dokument: 05_ANTI_PATTERN_v3.0.0.md (Quelle: .brain)

# ANTI-PATTERN: THE CEMETERY OF LEGACY ARCHITECTURE (v3.0.0)
# Status: CEMENTED | Doctrine: Chrome 147+ Baseline | Stand: M�rz 2026

## 1. DATA CORRUPTION (Persistence Anti-Patterns)
- **TOMB-P001:** Nutzung von `localStorage` f�r gro�e Datenmengen. (Grund: Synchroner I/O blockiert den Main Thread).
- **TOMB-P002:** Nutzung von Cloud-Synchronisation (Firebase/Supabase) f�r den Draft-State. (Grund: Versto� gegen Local-First).
- **TOMB-P003:** Nutzung von `JSON.stringify()` im Main Thread w�hrend der Eingabe. (Grund: UI-Jank).

## 2. DOM POLLUTION (Layout Anti-Patterns)
- **TOMB-L001:** Nutzung von `innerHTML` f�r Nutzer-Inhalte. (Grund: XSS-Risiko).
...
- **TOMB-L004:** Nutzung von CSS Frameworks (Tailwind/Bootstrap). (Grund: Unn�tiger Bloat).

## 3. LOGIC CONTAMINATION (Code Anti-Patterns)
- **TOMB-C001:** Nutzung des `Date` Objekts. (Grund: Mutabilit�t/Fehlerhaftigkeit. Ersetzt durch `Temporal`).
...
- **TOMB-L008:** Nutzung von `setTimeout` oder `setInterval` f�r Debouncing/Autosave. (Grund: Main-Thread Blockade; ersetzt durch Native Idle Detection API).

## 4. OBSOLETE TOOLS (Banned Tools)
- **TOMB-T001:** Nutzung von `head` und `tail` im Terminal. (Grund: Kontext-Fragmentierung).
- **TOMB-T002:** Nutzung von `cat <<EOF` in PowerShell. (Grund: Syntax-Inkompatibilit�t).

## 5. BUILD BLOAT (Preprocessor Anti-Patterns)
- **TOMB-V009:** Nutzung von CSS Pr�prozessoren (SASS, SCSS, LESS, Stylus). (Grund: Versto� gegen Zero-Build-Step Doktrin; native Chrome 147+ Features wie Nesting, Custom Properties und Math machen sie obsolet).
- **TOMB-M002:** Nutzung von SASS Mixins. (Grund: Code-Duplizierung und Build-Abh�ngigkeit; ersetzt durch native Custom Properties und atomare Komposition).
- **TOMB-V010:** JS-Scroll-Listener f�r Overflow-Feedback. (Grund: Ersetzt durch `@container scroll-state()` Queries in CSS).
...
## � Dokument: 06_SPEC_VALIDATION_ANCHOR_v1.0.0.md (Quelle: .brain)

# SPEC: THE BLACK-BOX-DECODER (VALIDATION OVERLAYS) (v1.0.0)
# Status: DRAFT | Doctrine: Core Grade Standard | Stand: M�rz 2026

## 1. NATIVE ANCHOR POSITIONING (Visual Tethering)
Validierungsfehler m�ssen physisch an das fehlerhafte IMR-Feld gebunden sein, ohne JS-Berechnungen.
- **Anchor Point:** Jedes `<din-*>` Element definiert einen eigenen `anchor-name` (z.B. `--anchor-body`, `--anchor-iban`).
- **Decoder Overlay:** Ein zentrales `<div popover id="black-box-decoder">` dient als Anzeige f�r Validierungsfehler.
...
- **Fragmentierung:** Nutzung von `position-visibility: anchors-visible` zur Vermeidung von Fehlern bei Seitenumbr�chen.
...
- [ ] 0 Zeilen JS-Code f�r die Positionsberechnung.
...
## � Dokument: 08_isomorphic_schema_v3.0.0.md (Quelle: .brain)
...
tags: [Core-grade, Standard-2026, ssot, imr, chrome-148, native-cma]
supersedes: [08_isomorphic_schema_v2.5.0.md, 08_isomorphic_schema_v2.2.0.md, 07_measurement_conflict_log.md]
authority: SUPREME SSoT f�r Datenstruktur, Geometrie und Interoperabilit�t.
---

# 08 � Isomorphic Master Registry (IMR) v3.0.0

## I. DIE VIER-EINHEITS-INVARIANTE (DOKTRIN)
Jedes funktionale Element des DIN-Briefs existiert in genau einer, deterministisch ableitbaren Form �ber alle Systemebenen hinweg. Jede Abweichung ist ein kritischer Architektur-Fehler.
...
Alle Koordinaten sind in Millimetern (mm) angegeben und basieren auf der **DIN 5008:2020-03** (Core Grade Validierung).
...
Das System nutzt das erweiterte `attr()` der CSS Values Level 5 zur direkten Typ-Konvertierung von HTML-Attributen in physische L�ngenma�e.

- **Implementierung:** `top: attr(data-cma-top type(<length>), 125mm);`
- **Vorteil:** Null JavaScript f�r die Layout-Berechnung. Die Browser-Engine (C++) �bernimmt die Millimeter-Pr�zision.
- **Circuit Breaker:** Ung�ltige Attribute (z.B. "103.4mx") fallen auf den im CSS deklarierten Standard-DIN-Wert zur�ck.

## IV. PLAINTEXT-ONLY DOKTRIN (ADR-008)
Alle `<din-*>` Felder tragen ausnahmslos das Attribut `contenteditable="plaintext-only"`.

- **MANDATE-INJ:** `innerHTML` ist f�r alle Briefdaten verboten.
- **GHOST-MIRROR:** Die einzige Ausnahme f�r visuelle Formatierung ist der `din-body-mirror`, der via `CSS Custom Highlight API` (::highlight) gerendert wird. Die Datenquelle bleibt 100% reiner Text (Markdown-Syntax).

## V. PROFILE & STAMMDATEN (AKINATOR-EXTENSIONS)
Erweiterte Keys f�r die Persistenz des Absender-Profils.
...
Das System ist intolerant gegen�ber Schema-Verschmutzung.
- **Vollst�ndigkeit:** Unbekannte Keys f�hren zur Ablehnung des Imports.
- **Mapping:** Es existiert kein Legacy-Mapping f�r v1-Keys.
- **Envelope:** Einzig erlaubter Nicht-IMR-Key ist `_meta` f�r Versions-Dokumentation.

## VII. VALIDIERTE DIN-KONSTANTEN (EX-CONFLICT-LOG)
Zementierte Werte zur Beilegung aller Mess-Konflikte:
1. **Betreff (SUBJECT_TOP):** 103.4 mm (Zentrum-Pr�zision).
2. **Infoblock (INFO_BLOCK_LEFT):** 125 mm (Harsh-Grid).
3. **Signatur (FOOTER_TOP):** 269 mm (Pr�zision vor Varianz).
...
## � Dokument: 12_akinator_logic_v2.0.0.md (Quelle: .brain)

---
tags: [Core-grade, Standard-2026, akinator, smart-paste, spec]
status: cemented
version: 2.0.0
last_audit: 2026-03-23
id: BRAIN-012-AKI
...
## I. INTENT
Sicherstellung der Datenwahrheit beim Import von unstrukturierten Texten (Impressum, E-Mails) durch den Akinator. Jede extrahierte Information muss eine nachpr�fbare Quelle im Original-Dokument besitzen.

## II. DER EVIDENCE-LINK PROZESS (SCHUTZ GEGEN HALLUZINATIONEN)
Das vom Akinator zur�ckgegebene JSON-Objekt darf keine isolierten Werte enthalten. Jedes IMR-Feld erfordert ein korrespondierendes `_source` Fragment.
...
~~~json
{
  ...
  "recipient_street": {
    ...
    "source_snippet": "Postanschrift: Gleisdreieck 1, 50667 K�ln"
  }
}
~~~

## III. FUNKTIONALER ABGLEICH (DER GUARD)
Bevor die Daten in den DOM (die `din-*` Tags) flie�en, erfolgt ein automatischer Abgleich:
1. **In-Text Verifikation:** Das System pr�ft mittels `indexOf()` oder Regex, ob `source_snippet` tats�chlich im Original-Paste-Buffer vorhanden ist.
2. **Warn-Indikator:** Falls ein Snippet fehlt oder der Wert (`value`) nicht im Snippet enthalten ist -> Das Feld wird im UI gelb markiert (Core-Warnung: "Nicht verifiziert").
3. **Korrektur-Modus:** Der Nutzer muss das Feld manuell best�tigen (Click-to-Verify), um die Warnung zu l�schen.
...
## � Dokument: 15_chrome145_todo_v1.0.0.md (Quelle: .brain)

---
tags: [Core-grade, Standard-2026, architecture, chrome-146, incremental]
status: cemented
version: 1.0.0
last_audit: 2026-03-21
id: BRAIN-015
title: Chrome 146+ Modernisierungs-TODO � Kein Legacy
mandate: Ziel-Browser Chrome 145+. ALLES was dort nativ geht wird nativ. Kein @supports-Guard n�tig.
---

# 15 � Chrome 146+ TODO: Vollst�ndige JS?Native Substitution

> Kein Bock auf Legacy. Kein @supports. Kein Polyfill.
> Chrome 145+ ist die Baseline. Was dort l�uft, wird implementiert.

---

## BLOCK A � SOFORT (0 Risiko, maximaler Gewinn)

### A-1: execCommand() L�SCHEN � Strategische Entfernung
...
- Strategischer Schutz: Entfernen verhindert, dass ein zuk�nftiger Dev
  plaintext-only zur�ckdreht um die Toolbar zu "reparieren" ? ADR-008 w�re gebrochen.
...
- Aktion: 1 EventListener l�schen, button durch label ersetzen
...
- Nutzen: din-*-Selektoren STRUKTURELL unm�glich au�erhalb #paper

### A-4: @starting-style f�r Toast-Animationen
- Datei: css/sidebar.css + js/ui/ui.js _toast()
- Was: JS opacity-Wechsel via setTimeout f�r Fade-In
- Ersatz CSS:
    .toast { opacity: 1; transition: opacity 0.3s; }
    @starting-style { .toast { opacity: 0; } }
- Chrome: 117+ Baseline 2024 � JS-Timer f�r Einblendung entf�llt
...
- Chrome: 129+ � 1 Zeile, global wirksam f�r alle Transitions

### A-6: field-sizing: content f�r Textarea
- Datei: css/sidebar.css + index.html #akinator-output
- Was: JS scrollHeight-Hack f�r Auto-Resize
- Ersatz: textarea { field-sizing: content; min-height: 3lh; }
- Chrome: 123+ � 1 Property, 0 JS

### A-7: Sanitizer API setHTML() f�r Ghost-Mirror
- Bei Ghost-Mirror-Implementierung direkt einbauen
- Chrome: 116+ (vollst�ndig 146+)
...
### A-8: @property f�r ALLE 14 CMA-Variablen
- Datei: css/din5008-paper.css
...
- Ersatz:
    ...
    (f�r alle 14 CMA-Konstanten)
- Chrome: 85+ (l�ngst stabil bei 145+)
- Nutzen: Browser-Validierung. Falsches Typo --subject-top: "103mm4" ? Browser-Fehler.
  Animierbar f�r zuk�nftige �berg�nge. Kein JS-Overhead.

### A-9: calc-size() f�r Akkordeon-Animationen
...
- Kein JS-H�hen-Rechner mehr

---

## BLOCK B � INVOKER COMMANDS (Chrome 133+)

Bei Chrome 145+ als Ziel kein Fallback n�tig f�r Chrome.
...
- JS addEventListener komplett entfernen f�r Standard-Dialog-Ops
...
### C-1: Gru�formel-Validierungshinweis neben din-greeting
- Aktuell: aria-invalid setzt nur outline � kein erkl�render Kontext-Hinweis
- Neu:
...
- Wenn din-body Inhalt l�nger als 1 Seite ? visueller Hinweis
- animation-timeline: scroll() auf Overflow-Badge
- Kein JS f�r Scroll-Position-Tracking

### E-2: Seiten-Progress-Bar (SPEC-029, mehrseitig)
- animation-timeline: scroll() f�r Seite-1/2-Fortschrittsanzeige
...
- Referenz: 08_isomorphic_schema_v2.2.0.md Sektion J (vollst�ndige Tabelle)
- Heute harmlos. Wenn Typed attr() GA: initCMABridge() l�schen.
...
JETZT (diese Session / n�chste Session):
  A-1  execCommand l�schen          ? Dead Code, 0 Risiko
  A-2  label f�r Import             ? 5 Minuten
...
ZUKUNFT (~Chrome 150):
  F-1  Typed attr() ? CMA-Bridge entf�llt komplett
...
## UNVERZICHTBARES JS � KEIN NATIVER ERSATZ (NIEMALS)
  ...
  validateIBAN()        BigInt Modulo-97 Pr�fsumme
...
Netto-Bilanz nach vollst�ndiger Umsetzung:
...
## � Dokument: 17_architecture_integrity_report_v1.0.0.md (Quelle: .brain)
...
last_audit: 2026-03-21
id: BRAIN-017
title: Architecture Integrity Report � Vollst�ndiger System-Audit
auditor: Lead Systems Auditor (Core Grade)
...
Jede Behauptung in diesem Report ist gegen die tats�chlichen Quelldateien
gepr�ft � constants.js, logic.js, die vier Brain-Docs. Keine Spekulation.
Befunde sind pr�fbar, Quellenangaben sind pr�zise.
...
~~~
KRITISCH  4 Befunde � Sofortige Korrektur n�tig
...
Die kritischen Befunde sind Dokumentations-Widerspr�che,
...
**Schwere:** KRITISCH � Kann LLM-Sessions und zuk�nftige Agenten fehlleiten.

**Befund:**
Die Datei `08_isomorphic_schema.md` (die ALTE Version, noch im .brain-Ordner)
enth�lt in Sektion H folgende JSON-Keys:
...
nicht existieren. Die Daten w�rden lautlos ignoriert.

**Quellen:** `08_isomorphic_schema.md` Sektion H (alt) vs. `constants.js` Z. 52�62

**Korrekturaktion:**
`08_isomorphic_schema.md` erh�lt einen SUPERSEDED-Header:
~~~
...
### K-002 � INTERNER WIDERSPRUCH: @supports-Guard f�r Anchor Positioning
...
> "CSS Anchor Positioning: Mit @supports-Guard verwenden (Firefox-Status pr�fen)"

`15_chrome145_todo_v1.0.0.md` (Block C, Pr�ambel):
> "Kein @supports bei 145+ als Ziel. Direkter Einsatz."
...
"Chrome Baseline 146+ � Kritische Einschr�nkung" ersetzen durch:
"Mandate Chrome 145+: Kein @supports n�tig f�r Anchor Positioning, Invokers,
Sanitizer API. Web Baseline ist f�r dieses Projekt nicht das Ziel."
...
Jeder Entwickler der sich an den Blueprint h�lt, implementiert eine falsche
...
Sektion "Schicht 2: ELEMENTS" anpassen: latex.base als neuen schw�chsten Layer.

---

### K-004 � ANTI-016-VERSTOSS: todayISO() nutzt noch new Date()

**Schwere:** KRITISCH � Aktiver Code verst��t gegen zementiertes Anti-Pattern.
...
> "Verbot: Nutzung der nativen new Date() Klasse f�r neue Logik."

`09_resilience_strategy_v4.0.0.md` Teil II:
> "Temporal.Now.plainDateISO() gibt immer YYYY-MM-DD zur�ck. Unverzichtbar
> f�r den Calendar-Day-Vergleich im Circuit Breaker."

Der Circuit Breaker BRAUCHT Temporal f�r seinen `lastStrikeDateISO`-Vergleich.
`todayISO()` liefert die Basis daf�r � aber noch mit `new Date()`.
Das ist ein direkter ANTI-016-Versto� und untergr�bt das Resilienz-Modell.
...
Fehlend (5 Felder ohne anchor-name):
  din-note        ? Vermerkzone � kein Anker f�r "Einschreiben"-Tooltip
  din-recipient   ? Empf�nger � kein Anker f�r Adress-Validierung
  din-your-ref    ? Ihr Zeichen � kein Anker f�r Format-Hinweis
  din-our-ref     ? Unser Zeichen � kein Anker f�r Format-Hinweis
  din-salutation  ? Anrede � kein Anker f�r Genus-Hinweis

Das Prinzip "TAG = KEY = KOORDINATE = CSS-ANKER" (4. Dimension) ist
f�r 45% der IMR-Felder NICHT vollst�ndig umgesetzt.

**Warum das wichtig ist:**
Tooltips f�r Recipient-Validierung (Pflicht-PLZ etc.) k�nnen ohne
...
`09_resilience_strategy_v4.0.0.md` definiert localStorage als SSoT f�r
den Circuit-Breaker-Zustand. Zwei Failure-Szenarien sind NICHT dokumentiert:

**Szenario A � localStorage geleert:**
Browser-Privacy-Einstellungen oder manuelles L�schen setzen alle Circuit-Breaker
auf GREEN. Ein Dienst der 13 von 14 Tagen ausgefallen war startet frisch.
Das ist SICHER (kein Datenverlust) aber UNERWARTET (Hysterese-Ged�chtnis weg).
Kein Nutzer-Hinweis. Kein Protokoll.

**Szenario B � QuotaExceededError:**
`state.js` hat: `try { localStorage.setItem(...) } catch { /* silent */ }`
Beim Circuit-Breaker-Update: Wenn die Phase-�nderung (z.B. AMBER ? RED)
nicht persistiert wird wegen QuotaError, ist der In-Memory-State (RED)
inkongruent mit dem localStorage-State (AMBER). Beim n�chsten App-Start:
System denkt der Dienst ist AMBER, resettet ihn nach einer Probe-Request �
obwohl er RED sein sollte. Das Circuit-Breaker-Ged�chtnis ist korrumpiert.

**Aktion:** `09_resilience_strategy_v5.0.0.md` � Recovery-Sektion erg�nzen:
...
1. Wie ist `din-body-mirror` positioniert? Es ist eine neue Custom-Tag.
   ...
   Wenn nein � die Elemente �berlagern sich nicht korrekt.
...
3. Die `din-body-mirror` f�hrt zu DOPPELTEM DOM:
   Inhalt existiert als Plaintext in `din-body` UND als HTML in `din-body-mirror`.
   Bei Mehrseitigkeit flie�en beide � aber nur einer wird gedruckt.
   Das Layouts-Reflow durch den Mirror k�nnte die DIN-Ma�e des restlichen
...
reader.onload = ev => {
  ...
  catch { this._toast('Ung�ltige Datei', 'error'); }
};
~~~

Kein IMR-Schema-Check. Ein JSON-Backup aus der IMR-v1-�ra
...
### G-005 � INTERMITTENT SERVICE: Reset-Logik sch�tzt nicht gegen Dauerst�rer

**Befund:**

Das Daily-Strike-Modell sch�tzt vor dauerhaft toten Diensten (14 Tage BLACK).
Es sch�tzt NICHT vor Diensten die chronisch instabil aber nie komplett tot sind.
...
Ein Dienst der t�glich ausf�llt, aber jeden zweiten Tag f�r 1 Probe-Request
antwortet, ERREICHT NIE BLACK. Das Hysterese-Modell ist unwirksam f�r
intermittente Dienste. Der Nutzer bekommt t�glich schlechte API-Erfahrung
...
oder �hnliches. Der ADR-Register in GEMINI.md endet bei ADR-009.

`08_isomorphic_schema_v2.2.0.md` Sektion M tr�gt in der traceability:
`[ADR-008, ADR-009, ADR-010, ...]` � ADR-010 referenziert aber keine
existierende Quelldatei.

Folge: Ein Agent kann "ADR-010" nicht aufl�sen. Die Entscheidung ist
de facto dokumentiert (Sektion M), aber nicht als eigenst�ndiger ADR zementiert.
...
`09_resilience_strategy_v4.0.0.md` Teil II:
> "Temporal.Now.plainDateISO() gibt immer YYYY-MM-DD zur�ck. �berall. Deterministisch."

Das ist technisch korrekt � aber es gibt eine Nuance:
`Temporal.Now.plainDateISO()` gibt das Datum in **UTC** zur�ck, nicht in der
lokalen Zeitzone des Nutzers.

Edge-Case: Nutzer in Tokyo (UTC+9), 08:00 Uhr Lokalzeit.
UTC ist 23:00 des Vortages. `plainDateISO()` gibt den Vortag zur�ck.
Ein Strike um 08:00 Ortszeit wird als "gestern" geloggt.
Am n�chsten Tag (Lokalzeit) stimmt der Vergleich wieder � aber f�r
diesen Nutzer deckt ein "Kalendertag" nur 15 Stunden Lokalzeit ab.

F�r das Daily-Strike-Modell ist das konservativ (gut) aber undokumentiert.
Die korrekte Implementation w�re:
`Temporal.Now.plainDateISO(Temporal.Now.timeZoneId())`
� gibt das Datum in der lokalen Zeitzone zur�ck.

**Aktion:** Spezifikation in 09 v5 pr�zisieren: Welche Zeitzone ist die Referenz?
...
### O-001 � Sektion L: Alle 11 Anker vollst�ndig dokumentieren
...
Dokumentation. Der n�chste IMR-Release (v2.3.0) sollte alle 11 vollst�ndig
dokumentieren mit spezifischen Tooltip-Szenarien:

| HTML-Tag       | anchor-name         | Tooltip-Nutzung (Beispiel)              |
|----------------|---------------------|-----------------------------------------|
| din-note       | --din-note          | Maximale Zeichenzahl f�r Vermerke       |
...
(the constitution), nicht BRAIN-015 (the todo list). Er w�rde @supports-Guards
implementieren, die nicht n�tig sind.

Priorit�t: `03_technical_blueprint_v1.1.0.md` erstellen � Chrome-145-Mandate
als erste Zeile, @supports-Empfehlungen streichen, latex.base-Layer erg�nzen.
...
Sofortige Massnahme (kein neues File n�tig):
...
~~~
?? SUPERSEDED � Diese Datei ist veraltet.
Nachfolger: 08_isomorphic_schema_v2.2.0.md
JSON-Keys in Sektion H sind falsch (sender_line ? sender etc.)
Nicht f�r Produktions-Prompts verwenden.
~~~

---

## BLOCK 4: KONSISTENZ-BEST�TIGUNG (was KORREKT ist)

Zur Vollst�ndigkeit: Diese Aspekte sind nachweislich konsistent.

### ? IMR-Code-Konsistenz

`constants.js` IMR (11 Eintr�ge) stimmt 1:1 mit `08_isomorphic_schema_v2.2.0.md`
Sektionen A�E �berein. Alle tag/key-Mappings korrekt. Tag-zu-Key-Formel
`tag.slice(4).replace(/-/g,'_')` ist in Code und Dokumentation identisch.

### ? ADR-008 vollst�ndig implementiert

`constants.js`: richText-Flag entfernt (TOMB-L008). ?
`logic.js`: readDOMasJSON() liest ausschlie�lich textContent. ?
Sektion K (Ghost-Mirror): Screen-State-Maschine vollst�ndig beschrieben. ?
ADR-008 ist in GEMINI.md, constants.js, logic.js und IMR konsistent.

### ? ADR-009 konsistent

`14_adr_009_ssot.md`: Vollst�ndige Begr�ndung. ?
IMR v2.2.0 Sektion I: "ADR-009: Prim�re SSoT ist css/din5008-paper.css" ?
...
### ? Silent Failure: Kein UX-Gap f�r Standard-APIs

F�r alle Tier-1-APIs (IBAN, Feiertage, Bundesbank) gibt es lokale Fallbacks.
Degradation ist transparent � der Nutzer kann weiterarbeiten.
Einzige problematische Ausnahme: Feiertagskalender (G-002-�hnlich, aber
spezifisch f�r Fristenberechnungen mit veralteter Statik-Tabelle).
...
ber�hrt KEINE Layout-Properties (position, top, left, width, height).
...
~~~
PRIORIT�T 1 � SOFORT (n�chste Session):
...
PRIORIT�T 2 � N�CHSTE SPEC-RUNDE:
...
PRIORIT�T 3 � LANGFRISTIG (eigene SPECs):
  ...
  G-007  Temporal Timezone-Referenz pr�zisieren (09 v5)

OPTIMIERUNGEN (keine Priorit�t, bei n�chster Ber�hrung):
  O-001  Sektion L vervollst�ndigen (Teil von G-001)
...
Dokumentations-Widerspr�che, kein Laufzeit-Fehler.

Der schwerste Einzelbefund ist K-001 (alte Schema-Datei mit falschen Keys) �
eine Quelle von Fehlinformation f�r zuk�nftige Agenten und LLMs.
Er ist in 5 Minuten behoben.

Der systemic wichtigste Befund ist K-004 (todayISO mit new Date()) �
der einzige Fall wo aktiver Code gegen ein zementiertes Anti-Pattern verst��t.

Nach Abarbeitung von Priorit�t 1 ist das Fundament versiegelt.


---


## � Dokument: 18_logic_migration_temporal_v1.0.0.md (Quelle: .brain)

---
tags: [Core-grade, Standard-2026, architecture, gap-healing, chrome-146]
status: cemented
version: 1.0.0
last_audit: 2026-03-21
id: BRAIN-018
...
heals: [K-004 new Date() ANTI-016 Versto�, G-007 UTC vs. Local Timezone]
---

# 18 � Logic Migration: Temporal API f�r js/logic/logic.js

## Kontext

BRAIN-017 K-004 identifizierte: `todayISO()` in `js/logic/logic.js` nutzt
`new Date()` � ein aktiver Versto� gegen ANTI-016 (Verbot der nativen Date-Klasse
f�r neue Logik). Gleichzeitig identifizierte G-007 ein Timezone-Problem in der
Temporal-Nutzung der Resilience-Strategie.

Diese Datei versiegelt die korrekte Spezifikation f�r beide Punkte.
...
1. **Mutabilit�t:** Ein `Date`-Objekt ist mutable. `d.setMonth(0)` ver�ndert
   ...
   Objekt zur�ck, das Original bleibt unver�ndert.

2. **Locale-Abh�ngigkeit:** `toLocaleDateString()` gibt systemabh�ngige Formate
   zur�ck. Auf deutschen Systemen `"21.03.2026"`, auf US-Systemen `"3/21/2026"`.
   Ein String-Vergleich mit einem ISO-gespeicherten Wert aus dem LocalStorage
   w�rde in 30% aller Systeme silently falsch vergleichen.

3. **Timezone-Ambiguit�t:** `new Date()` kennt keine "Kalender-Tag"-Semantik.
   `new Date().toISOString()` gibt UTC zur�ck. Der Nutzer in Tokyo der um
   08:00 Ortszeit (= 23:00 UTC Vortag) einen Brief schreibt bekommt das
   gestrige Datum � der Hysterese-Counter z�hlt falsch.

4. **ANTI-016 Versto�:** Dokumentiert in `05_anti_pattern_registry.md`.
   Jede neue Logik darf `new Date()` nicht verwenden.

**Status:** TOMB-L001 � Begraben ab 2026-03-21.
**Gilt f�r:** `todayISO()`, alle `new Date()` Konstruktoren in logic.js.
**Ausnahme:** Bestehendes `parseDate()` darf `new Date(string)` noch nutzen
als tempor�re Br�cke � aber nur f�r das Parsen von Nutzereingaben.
...
- **Eingabe:** keine
- **Ausgabe:** String im Format `YYYY-MM-DD` � unver�nderlich, deterministisch
- **Zeitzone:** Lokale Zeitzone des Nutzers (NICHT UTC)
- **Kompatibilit�t:** R�ckgabewert ist direkt als LocalStorage-Key nutzbar

### Das Timezone-Problem (G-007) � "Midnight Bug"

`Temporal.Now.plainDateISO()` ohne Argument gibt das Datum **in UTC** zur�ck.

Konsequenz f�r das Daily-Strike-Modell:
~~~
Nutzer in Tokyo (UTC+9) � Montagmorgen 08:00 Lokalzeit
UTC: Sonntag 23:00
plainDateISO() gibt zur�ck: "2026-03-22" (Sonntag in UTC)
LocalStorage erwartet: "2026-03-23" (Montag in Lokalzeit)
? Der Strike-Counter glaubt, es ist noch Sonntag.
? Der Montags-Strike z�hlt f�lschlich als zweiter Sonntags-Strike.
~~~

Das ist der Midnight Bug: 9 Stunden pro Tag in Tokyo sind "falsch".
F�r Deutschland (UTC+1/+2) betrifft es 1�2 Stunden um Mitternacht.
Selten, aber deterministisch falsch wenn es passiert.

### Die Korrekte Spezifikation

Die lokale Zeitzone muss explizit �bergeben werden:
...
2. `Temporal.Now.plainDateISO(timeZoneId)` gibt das aktuelle Datum
   in dieser Zeitzone zur�ck � als reinen Kalendertag ohne Zeit
3. Ergebnis: `"2026-03-21"` � immer der lokale Kalendertag des Nutzers

**R�ckgabeformat:** `YYYY-MM-DD` � identisch zur bisherigen Implementierung.
Keine Breaking Changes am LocalStorage-Schema. Kein Migrations-Aufwand.

### Scope der Migration

**Betrifft direkt:**
- `todayISO()` in `js/logic/logic.js` � prim�re Baustelle
...
**Betrifft NICHT:**
- `parseDate(input)` � darf weiterhin `new Date(string)` f�r Input-Parsing nutzen
  (Tier-3 Fallback, nicht f�r Zeitstempel-Generierung)
- ISO-Timestamps f�r `lastSuccessTs`, `deadFlagTs`, `probeLastTs` im
  LocalStorage-Schema � diese nutzen `Temporal.Now.instant().toString()` (UTC ISO-8601)

### Temporal Polyfill � Einbindungs-Anforderung

Temporal ist in Chrome 145+ nativ verf�gbar (Chrome 127+ mit Flag, stable ab ~130).
Bei unserem Ziel-Browser Chrome 145+ ist kein Polyfill n�tig.

**Konditionale Logik:** Falls ein Polyfill ben�tigt wird (z.B. f�r Testing-Umgebungen):
`@js-temporal/polyfill` � einmalig in `index.html` via `importmap` oder
direkt in `app.js` vor dem ersten `Temporal`-Aufruf.

---

## LocalStorage-Kompatibilit�t: Kein Breaking Change

Das bestehende Schema speichert `lastStrikeDateISO` als `YYYY-MM-DD`.
Die neue `todayISO()` gibt weiterhin `YYYY-MM-DD` zur�ck.
Ein O(1) String-Vergleich (`lastStrikeDateISO === todayISO()`) bleibt korrekt.

Einzige Verhaltens�nderung: F�r Nutzer mit UTC-abweichender Zeitzone werden
...
2. `js/core/constants.js` oder `js/app.js`: Temporal verf�gbar pr�fen
3. `05_anti_pattern_registry.md`: TOMB-L001 als Cemetery-Eintrag erg�nzen
...
## � Dokument: 19_final_cleanup_audit_and_cemetery_v3.0.0.md (Quelle: .brain)

---
tags: [Core-grade, Standard-2026, architecture, legacy-purge, cemetery, guardrails]
status: cemented-pending-approval
version: 3.0.0
last_audit: 2026-03-21
id: BRAIN-019-TOMB
traceability: [BRAIN-017, BRAIN-018, BRAIN-008, BRAIN-015, BRAIN-003, MANDATE-NO-LEGACY]
holy_folder: "C:\\Users\\morit\\Documents\\antigravity\\DIN Brief\\Standard_DIN_2026"
...
Dieser Report ist das Ergebnis eines vollst�ndigen Code-to-Spec-Alignment-Scans.
Jeder Befund basiert auf dem tats�chlich gelesenen Dateiinhalt.
Keine Spekulation. Keine Halluzination. Quellenangaben sind pr�zise.
...
## KATEGORIE 1: DESTRUCTION LIST � Sofort l�schbarer Dead Code
...
ADR-009 hat CSS als prim�re SSoT zementiert. Die Bridge-Funktion ist Dead Code.
...
**L�sch-Anweisung:**
1. `app.js`: Import-Zeile `import { initCMABridge } from './cma-bridge.js'` ? l�schen
2. `app.js`: `initCMABridge(storedForm)` Aufruf + Kommentar ? l�schen
3. `cma-bridge.js`: `initCMABridge()` Funktion komplett ? l�schen
4. `cma-bridge.js`: Export nur noch `switchForm` ? anpassen
5. Vor L�schung: `/* DEAD: See TOMB-B001 */` Marker setzen
...
Die Buttons klicken ins Leere. Dead Code mit T�uschungspotenzial.

**L�sch-Anweisung:**
1. `ui.js`: gesamter `querySelectorAll('[data-cmd]')` Block ? l�schen
2. `index.html`: die drei `data-cmd` Buttons ? l�schen (Undo/Redo-Buttons behalten!)
3. Vor L�schung: `/* DEAD: See TOMB-EXEC-001 */` Marker setzen
...
daher immer `false/undefined`. `_safeSetHTML()` kann �ber diese Pfade nie
legitim aufgerufen werden. Zus�tzlich: unkontrolliertes `innerHTML =` ist
ein MANDATE-INJ-Versto�.

**L�sch-Anweisung:**
1. `ui.js`: `_safeSetHTML()` Methode ? l�schen
...
4. Vor L�schung: `/* DEAD: See TOMB-U002 */` Marker setzen
...
Nach D-003 (L�schen von `_safeSetHTML`) werden diese Stellen ohnehin vereinfacht.

**L�sch-Anweisung:** Ternary ? direkter `textContent`/`innerText`-Pfad. `/* DEAD: See TOMB-L008 */`
...
**Befund:** `btn-import-trigger` existiert nur, um `file-import.click()` auszul�sen.
Das ist die klassische HTML-Antipattern f�r File-Inputs.
Native Alternative: `<label for="file-import">` �bernimmt den Klick ohne JS.

**Umstellungs-Anweisung:**
1. `index.html`: `<button id="btn-import-trigger">` ? `<label for="file-import" ...>?? Import</label>`
2. `ui.js`: `on('btn-import-trigger', ...)` Zeile ? l�schen
3. `file-import` `hidden` ? `display:none` via CSS (label-click funktioniert unabh�ngig)
...
**Chrome 133+ stabil. Kein @supports n�tig bei Chrome 145+ Mandate.**
...
**Befund:** Aktiver ANTI-016-Versto�. TOMB-L001 best�tigt. Muss zu
...
**Befund:** Zweite `new Date()`-Verwendung f�r Zeitstempel-Generierung.
ANTI-016 Versto�. `new Date()` f�r "heute" ist TOMB-L001-Scope.
...
**Befund:** Inline-JS widerspricht der No-JS-Doctrine f�r neue Entwicklungen.
Form-A/B Toggle geh�rt in `ui.js:_bindActions()` per Event-Delegation.
...
zementiert: `latex.base` als schw�chster Layer. Die Spezifikation ist
korrekt, der CSS-Code wurde noch NICHT aktualisiert.
Aktueller Stand ist ein Spec-Code-Divergenz (kein Laufzeitfehler, da
latex.base noch keine Regeln enth�lt, aber die Deklaration fehlt).
...
### N-007 | @property f�r 14 CMA-Variablen fehlt | BRAIN-015 A-8
...
**Fundstelle:** `css/sidebar.css` f�r `#akinator-output` Textarea.

**Befund:** `field-sizing: content` ist f�r Auto-Resize der Akinator-Textarea spezifiziert
...
**Befund:** ADR-008 ist unmissverst�ndlich: ALLE `<din-*>`-Tags tragen ausnahmslos
`contenteditable="plaintext-only"`. `din-body` ist KEINE Ausnahme.
Formatierungen laufen �ber den Ghost-Mirror (`<din-body-mirror>`), nicht
�ber `contenteditable="true"`.

Dies ist die einzige aktive ADR-008-Verletzung im Live-DOM.
Der Kommentar verst�rkt die Falschaussage.
...
(korrekt f�r ein neues Dokument). Wenn der Nutzer tippt, werden die richtigen IMR-Keys
in den State geschrieben (via `_bindTagInputs()`). Nach Save/Reload funktioniert alles.
Die falschen Keys sind Dead Weight � aber Tick-Zeitbomben f�r zuk�nftige
...
Der DOM enth�lt dieses Element nicht. Solange der Ghost-Mirror nicht existiert:
...
**Befund:** IMR v2.3.0 Sektion K spezifiziert `break-inside: avoid` f�r
...
Das Layout wird �ber `#paper[data-layout]` gesteuert. Das Attribut am `<html>` ist
ein �berbleibsel einer fr�heren Architektur und hat null Funktion.

**Fix-Anweisung:** `data-layout="form-b"` aus `<html>` entfernen.

---

### I-006 | state.js catch � zu breite Fehler-Unterdr�ckung | G-002
...
**Befund:** Der Kommentar nennt `quota exceeded`, aber `catch` ohne Typ f�ngt ALLES �
auch `JSON.stringify`-Fehler (circular references), TypeError usw.
09_resilience_strategy_v6.0.0.md Teil V spezifiziert eine Priorit�ts-Hierarchie
...
f�r strukturelle Isolation der `din-*` Selektoren. Nicht implementiert.
Chrome 118+ stabil. Kein Implementierungshindernis.

**Anweisung:** SPEC-IMPLEMENTATION-PENDING. N�chste CSS-Session.
...
- **Cause of Death:** ADR-009 � CSS `:root {}` ist die prim�re SSoT f�r alle
  ...
  ein Wartungsrisiko (2 Stellen m�ssen synchron gehalten werden).
- **The Successor:** CSS `din5008-paper.css :root {}` direkt. `switchForm()` bleibt
  als einzige legitime Bridge-Funktion f�r den dynamischen Form-A/B-Toggle.
...
- **Cause of Death:** ADR-008 � `contenteditable="plaintext-only"` blockiert
  ...
  Die Buttons sind visuell vorhanden aber funktionslos. T�uschungsgefahr.
- **The Successor:** Ghost-Mirror-Pattern f�r Markdown-Formatierung im Body.
...
- **Cause of Death:** TOMB-L008 (richText-Flag entfernt) + MANDATE-INJ (innerHTML
  f�r User-Content verboten). Die Methode kann nach TOMB-L008 �ber keine
...
- **The Successor:** `element.setHTML()` mit Sanitizer API (Chrome 116+, vollst�ndig 146+)
  f�r den Ghost-Mirror-Renderer � sobald dieser implementiert wird.
  Bis dahin: keine Ersetzung n�tig (Dead Code).
- **Status:** Code noch vorhanden. **D-003 ist der Deletion-Ticket.**
- **Epitaph:** `/* DEAD: See TOMB-U002 � MANDATE-INJ: innerHTML f�r User-Content verboten */`
...
- **Cause of Death:** IMR v2.0 (PLAN-010) hat die Keys umbenannt zu
  ...
  signalisieren f�lschlich ein falsches State-Schema an zuk�nftige Leser.
...
- **Cause of Death:** ADR-008 (2026-03-20) � Ausnahmen f�r `din-body` wurden
  explizit abgeschafft und in TOMB-L008 beerdigt. Der falsche Kommentar
  ist eine Einladung zur Regression � jeder zuk�nftige Dev w�rde denken,
...
- **Cause of Death:** Das Attribut hat keine CSS-Selektoren und keine JS-Logik.
  Layout wird ausschlie�lich �ber `#paper[data-layout]` gesteuert.
  Das `<html>`-Attribut ist ein �berbleibsel aus einer fr�heren Architektur-Iteration.
- **The Successor:** Kein Ersatz n�tig. Einfach entfernen.
...
### TOMB-L001 | new Date() f�r Zeitstempel-Generierung (BEST�TIGT)

- **ID:** TOMB-L001
- **Deceased Pattern:** `new Date()` und `Date.now()` f�r Datumsberechnung/Zeitstempel
- **Fundstellen im Code:**
  1. `logic.js:todayISO()` � `const d = new Date()` (PRIM�R)
  2. `app.js` Boot � `formatDate(new Date(), fmt)` (SEKUND�R)
- **Cause of Death:**
  1. Mutabilit�t: `Date`-Objekte sind mutable ? ANTI-016
  2. UTC-Fehler: `new Date().toISOString()` gibt UTC zur�ck ? Midnight-Bug (G-007)
  3. Locale-Abh�ngigkeit: `toLocaleDateString()` systemabh�ngig ? deterministisch falsch
- **The Successor:** `Temporal.Now.plainDateISO(Temporal.Now.timeZoneId())` � deterministisch,
  lokal, immutable, immer `YYYY-MM-DD`
- **Ausnahme:** `parseDate()` darf `new Date(string)` f�r User-Input-Parsing behalten
  (BRAIN-018 explizite Erlaubnis als Tier-0 Fallback).
- **Status:** Code noch vorhanden. **N-003 und N-004 sind die Migration-Tickets.**
- **Epitaph:** `/* DEAD: See TOMB-L001 � ANTI-016: new Date() f�r Timestamps verboten. Nutze Temporal. */`
...
- **ID:** TOMB-L007
- **Status:** VOLLST�NDIG ENTFERNT. Nur in Kommentaren referenziert.
...
- **ID:** TOMB-L008
- **Deceased Pattern:** `richText: true` Flag im IMR-Array + `innerHTML`-Nutzung f�r Body
...
- **Vollst�ndige Bereinigung:** D-003 + D-004 abarbeiten.
...
- **Cause of Death:** Diese Feld-Namen wurden nie in Produktions-Backups geschrieben.
  Die Migrations-Tabelle war pr�ventive Komplexit�t f�r ein nicht-existentes Problem.
...
- **ID:** TOMB-C001
- **Status:** VOLLST�NDIG ENTFERNT. Ersatz: `#paper[data-guides="true"]` CSS-Selektor.
...
- **ID:** TOMB-C002
- **Status:** VOLLST�NDIG ENTFERNT. Ersatz: `:has([contenteditable]:focus-within)` CSS.
- **Quelle:** css/GEMINI.md Cemetery-Eintrag.
...
### TOMB-C004 | margin-top f�r DIN-Zonen (ABGELEHNT)
...
| `/^(\d{1,2})\.(\d{1,2})\.(\d{4})$/` | Datum-Parse | SICHER � vollst�ndig anchored |
| `/^(\d{4})-(\d{2})-(\d{2})$/` | ISO-Datum | SICHER � vollst�ndig anchored |
| `/^(\d{1,2})\.?\s+([^\s]+)\s+(\d{4})$/` | Long-Datum | SICHER � [^\s]+ in kontrolliertem Kontext |
| `/\s+/g`, `/.{1,4}/g` | IBAN-Format | SICHER � keine User-Input-Injection m�glich |
| `/[^A-Za-z0-9]/g` | IBAN-Clean | SICHER � Whitelist-Ansatz |

**Kein ReDoS-Risiko** bei den gepr�ften Patterns. Alle quantifiers sind bounded
oder in anchored Contexts. kein catastrophic backtracking m�glich.
...
Aber ihre blo�e Existenz ist ein Risiko: ein zuk�nftiger Entwickler
k�nnte sie direkt aufrufen. Kein aktives XSS-Risiko im aktuellen Stand,
da kein Codepfad sie mit User-Input aufruft.

**Beweis:** `_onContentChange()` und `_syncAllToDOM()` haben `entry.richText`
als Guard � der ist immer `undefined` (falsy). Methodenaufruf nie ausgel�st.

**Empfehlung:** D-003 (L�schung) hat h�chste Sicherheitspriorit�t.
...
muss `setHTML()` der einzige Schreib-Pfad f�r Mirror-HTML sein.
`innerHTML =` auf `din-body-mirror` ist unter allen Umst�nden verboten.

---

## IMPLEMENTIERUNGS-CHECKLISTE � N�chste Session

### SOFORT (keine Spezifikation n�tig, 0 Risiko)

~~~
[ ] D-001  app.js: initCMABridge() Import + Aufruf l�schen
[ ] D-001  cma-bridge.js: initCMABridge() Funktion l�schen
[ ] D-002  ui.js: execCommand-Block l�schen
[ ] D-002  index.html: data-cmd Buttons (3x) l�schen
[ ] D-003  ui.js: _safeSetHTML() Methode l�schen
...
[ ] N-001  ui.js: entsprechenden EventListener l�schen
[ ] N-002  index.html: 6x popovertarget ? commandfor/command migrieren
[ ] N-006  css/din5008-paper.css: @layer latex.base hinzuf�gen
~~~
...
[ ] N-007  css: @property f�r alle 14 CMA-Variablen
[ ] N-008  css/sidebar.css: field-sizing: content f�r Textarea
[ ] I-007  css/din5008-paper.css: @scope (#paper) hinzuf�gen
~~~
...
## LEGACY GATE GUARDRAIL (UNVER�NDERLICH)

> **Vor JEDEM zuk�nftigen Vorschlag MUSS diese Registry gepr�ft werden.**
>
> Ein Vorschlag der ein beerdigtes Pattern wiederbelebt gilt als sofortiger
> Architecture-Audit-Failure. Keine Ausnahmen.
>
> **BEGRABEN:** innerHTML, richText, execCommand, new Date() f�r Timestamps,
> initCMABridge(), Legacy-Key-Mapping, Pico-CSS-Full-Import, margin-top f�r
> DIN-Zonen, ID-basierte Felder-Selektoren, JS-Klassen-Toggle f�r Sichtbarkeit.
>
> **CEMETERY ID-Nummern f�r Code-Marker:**
...
Datum:          2026-03-21
Auditor:        Lead Systems Auditor (Core Grade)
Scan-Methode:   Vollst�ndige Dateilekt�re aller relevanten Quellen
Gefundene Bugs: 2 aktive (I-001 ADR-008-Verletzung, I-002 State-Schema)
Dead Code:      4 identifizierte Bl�cke (D-001 bis D-004)
...
## � Dokument: 20_integrity_audit_v1.0.0.md (Quelle: .brain)
...
traceability: [BRAIN-017, BRAIN-018, BRAIN-008, BRAIN-015, BRAIN-003, MANDATE-NO-LEGACY]
corrections: [I-002 herabgestuft, TOMB-B002 neu, TOMB-GHOST-001 neu, I-001 pr�zisiert]
---
...
Korrigiert: 7 pr�zise Kritikpunkte (externe Peer-Review).
Methodik: Vollst�ndiger Codebefund bleibt identisch.
Korrekturen: Bewertungen, Sch�rfe, neue TOMBs.

**KORREKTUREN GEGEN�BER v3.0.0:**
...
3. I-001: Positiver Kontext erg�nzt (10/11 Compliance)
...
7. Inline-Handler, popovertarget, @property/@scope/field-sizing: Versch�rft
...
ADR-009 erkl�rt CSS als prim�re SSoT. Diese Funktion ist Dead Code.

**L�sch-Anweisung:**
1. `app.js`: Import-Zeile l�schen
2. `app.js`: `initCMABridge(storedForm)` Aufruf + Kommentar l�schen
3. `cma-bridge.js`: `initCMABridge()` Funktion l�schen
4. `/* DEAD: See TOMB-B001 */` vor L�schung setzen
...
`execCommand` auf plaintext-only ist ein No-Op. Die Buttons t�uschen Funktionalit�t vor.
T�uschung + Dead Code + Einladung zur ADR-008-Regression.

**L�sch-Anweisung:**
1. `ui.js`: gesamter `querySelectorAll('[data-cmd]')` Block l�schen
2. `index.html`: 3x `data-cmd` Buttons l�schen (Undo/Redo behalten!)
...
**Befund:** Unkontrolliertes `innerHTML =` ohne Sanitizer � MANDATE-INJ-Versto�.
Methodenaufruf ist dead code (entry.richText immer undefined).
Existenz allein ist Sicherheitsrisiko � zuk�nftiger Entwickler k�nnte sie missbrauchen.
**H�chste Sicherheitspriorit�t im gesamten Dead-Code-Bestand.**

**L�sch-Anweisung:** Methode l�schen. `/* DEAD: See TOMB-U002 � MANDATE-INJ */`
...
**L�sch-Anweisung:** Ternary vereinfachen zu direktem `_safeSet()`.
`/* DEAD: See TOMB-L008 */`
...
**Befund (korrigiert gegen�ber v3.0.0):**
...
unabh�ngig vom tats�chlichen Zustand.

Ein Attribut das beim App-Start `form-b` zeigt und beim Wechsel zu Form-A
nicht aktualisiert wird ist schlimmer als gar kein Attribut: Es l�gt.

**L�sch-Anweisung:** `data-layout="form-b"` aus `<html>` entfernen.
...
`<label for="file-import">` �bernimmt den Klick ohne JS.
...
| Button | Jetzt | Soll |
|--------|-------|------|
| Debugger �ffnen | `popovertarget="sidebar-right"` | `commandfor="sidebar-right" command="show-popover"` |
| Profil �ffnen | `popovertarget="dialog-profile"` | `commandfor="dialog-profile" command="show-popover"` |
| Inspector �ffnen | `popovertarget="sidebar-right"` | `commandfor="sidebar-right" command="show-popover"` |
| Sidebar schlie�en ? | `popovertargetaction="hide"` | `command="hide-popover"` |
| Dialog Abbrechen | `popovertargetaction="hide"` | `command="hide-popover"` |
| Dialog schlie�en ? | `popovertargetaction="hide"` | `command="hide-popover"` |
...
**Fundstelle:** `logic.js:todayISO()` � `new Date()` f�r Datumsberechnung (PFLICHT-MIGRATION)
...
Begr�ndung: `parseDate()` ist Tier-0-Fachlogik f�r das Parsen von
Nutzer-Input-Strings � kein Zeitstempel-Generator. Die Temporal-Migration
betrifft ausschlie�lich Funktionen die "jetzt" bestimmen, nicht solche
...
Inline-Handler vollst�ndig entfernen.
...
`@layer latex.base, din.core, ui.theme, project.overrides;`
Spec und Code divergieren. Kein Laufzeitfehler (latex.base enth�lt noch keine Regeln),
aber die fehlende Deklaration muss vor dem ersten latex.base-Inhalt gesetzt werden.

---

### N-006 | @property f�r 14 CMA-Variablen fehlt � BRAIN-015 A-8
...
**Befund:** JS-scrollHeight-Hack f�r Auto-Resize. Native Alternative:
...
ADR-009 sagt: CSS ist die prim�re SSoT f�r CMA-Koordinaten.
...
Die CSS-L�sung existiert bereits in `@layer project.overrides`:
~~~css
...
**Die saubere L�sung:** Nur `paper.dataset.layout` setzen (per Event-Delegation
aus dem Radio-Button). CSS `project.overrides` �bernimmt den Rest automatisch.
`switchForm()` wird zu einem 1-Zeiler der nur `dataset.layout` setzt �
oder entf�llt ganz.

**L�sch-Anweisung:** `setProperty('--address-top', ...)` aus `switchForm()` l�schen.
`switchForm()` auf reines `paper.dataset.layout = ...` reduzieren oder eliminieren.
Wenn eliminiert: `cma-bridge.js` Datei komplett l�schen + Import in `app.js` entfernen.
`/* DEAD: See TOMB-B002 � ADR-009: CSS project.overrides ist SSoT f�r Form-Toggle */`
...
Der Kommentar `AUSNAHME: ... (formatierter Text erlaubt)` ist eine L�ge im Live-DOM.
Er ist eine Einladung zur Regression � jeder k�nftige Entwickler der ihn liest
wird denken, rich text sei architektural erlaubt.

**Positiver Kontext (in v3.0.0 fehlend):**
10 von 11 `<din-*>`-Feldern sind bereits vollst�ndig ADR-008-compliant:
...
**Befund (KORRIGIERT gegen�ber v3.0.0 � war: "KRITISCH / Tick-Zeitbombe"):**

Das ist kein Laufzeit-Risiko und keine Tick-Zeitbombe. Die Bewertung in v3.0.0 war �bertrieben.
...
- Nach Save/Reload: `loadFromStorage()` l�dt gespeicherte echte Keys � die falschen
...
**Tats�chliches Problem:** Code-Smell + Documentation Debt.
Jemand der `DEFAULT_STATE` als Schema-Referenz liest, erh�lt ein falsches Bild.
Ein neuer Entwickler k�nnte `signatureName` in Prompts oder Tests verwenden.

**Bewertung:** TECHNISCHE SCHULDEN � kein aktives Risiko, keine Eile.
Neu: TOMB-STATE-002 (umbenannt, da TOMB-STATE-001 urspr�nglich falsch bewertet).
...
**Befund (KRITISCH HOCHGESTUFT gegen�ber v3.0.0):**
...
1. **Print-Layout ist NICHT DIN-5008-konform** f�r Markdown-Body-Inhalte.
   Wenn `din-body` `contenteditable="plaintext-only"` tr�gt (nach I-001-Fix)
...
2. **ADR-008 Ghost-Mirror-Pflicht ist unerf�llt.**
...
### I-004 | break-inside fehlt in @media print | KRITISCHER GAP (mit I-003 verkn�pft)
...
**Befund:** F�ngt alles. QuotaExceededError-Recovery aus `09_resilience_strategy_v6.0.0.md`
nicht implementiert. SPEC-IMPLEMENTATION-PENDING.


---

## KATEGORIE 4: CEMETERY � Zentrales Register + Neue Gr�ber
...
- **Successor:** Reines `paper.dataset.layout = 'form-a'`. CSS �bernimmt den Rest.
- **Status:** Code vorhanden ? N-009
- **Epitaph:** `/* DEAD: See TOMB-B002 � ADR-009: CSS project.overrides ist SSoT f�r Form-Toggle */`
...
- **Successor:** Ghost-Mirror-Markdown f�r `din-body`. Toolbar: Undo/Redo only.
...
- **Cause of Death:** TOMB-L008 (richText entfernt) + MANDATE-INJ (innerHTML f�r User-Content)
- **Successor:** `element.setHTML()` mit Sanitizer API f�r Ghost-Mirror-Renderer.
...
- **Cause of Death:** `<din-body-mirror>` existiert nicht im DOM.
  ...
  Papier ohne Substanz. Print-Layout f�r Markdown ist nicht DIN-5008-konform.
- **Successor:** Vollst�ndige Implementierung: `<din-body-mirror aria-hidden="true">` im DOM,
  State-Maschine (Fokus/Blur/Print), `setHTML()` mit Sanitizer API f�r Render-Pfad.
...
- **Cause of Death:** ADR-008 kennt keine Ausnahmen f�r `din-body`. Falsch und regressionsf�rdernd.
- **Status:** Im Live-DOM ? I-001
- **Epitaph:** `<!-- TOMB-HTML-001: ADR-008 gilt ohne Ausnahme -->`

---

### TOMB-HTML-002 | html[data-layout] L�gen-Attribut | No-JS-Doctrine

- **Deceased:** `<html lang="de" data-layout="form-b">`
- **Cause of Death:** Wird nie aktualisiert wenn Nutzer Form wechselt. L�gt dauerhaft.
...
### TOMB-L001 | new Date() f�r Zeitstempel (mit Ausnahme) | ANTI-016

- **Deceased:** `new Date()` f�r "heute" in `logic.js:todayISO()` + `app.js` Boot
- **Cause of Death:** ANTI-016, Mutabilit�t, UTC-Midnight-Bug (G-007), Locale-Abh�ngigkeit
- **Successor:** `Temporal.Now.plainDateISO(Temporal.Now.timeZoneId())`
- **Ausnahme BRAIN-018:** `parseDate()` f�r User-Input-Parsing � bleibt. Kein Zeitstempel.
- **Status:** Code vorhanden ? N-003
- **Epitaph:** `/* DEAD: See TOMB-L001 � Nutze Temporal. Ausnahme: parseDate(). */`

---

### TOMB-L007 | getElementById(domId) � IMR v1 | VOLLST�NDIG ENTFERNT ?
...
### TOMB-C001 | .is-visible Klassen-Toggle | VOLLST�NDIG ENTFERNT ?
### TOMB-C002 | JS-Toolbar show/hide | VOLLST�NDIG ENTFERNT ?
...
Alle Patterns in `logic.js` sind vollst�ndig anchored, bounded, kein ReDoS-Risiko.

### SEC-002 | _safeSetHTML() � Latentes XSS | H�chste L�sch-Priorit�t
...
ist der EINZIGE erlaubte HTML-Schreib-Pfad f�r den Mirror. `innerHTML =` ist verboten.
...
[ ] D-001  app.js + cma-bridge.js: initCMABridge() l�schen
[ ] D-002  ui.js + index.html: execCommand-Block + data-cmd Buttons l�schen
[ ] D-003  ui.js: _safeSetHTML() l�schen                                  ? SICHERHEITS-PRIORIT�T
...
### N�CHSTE RUNDE � Spec-Debt + Technische Schulden
...
[ ] N-009  cma-bridge.js: switchForm() setProperty l�schen / Datei eliminieren
[ ] I-002  state.js: 4 falsche DEFAULT_STATE Keys korrigieren
[ ] N-005  css/din5008-paper.css: @layer latex.base hinzuf�gen
[ ] N-006  css: @property f�r 14 CMA-Variablen
...
DEAD CODE:        5 Bl�cke (D-001 bis D-005)
...
HOCHGESTUFT:      I-003/I-004 (war PENDING, ist KRITISCHER GAP)
                  D-005 (war KURZ-ERW�HNT, ist DESTRUCTION LIST)
                  N-009 switchForm() (war LEGITIM, ist KRITISCH)

SICHERHEIT:       Kein aktives XSS. _safeSetHTML() latent ? D-003 h�chste Prio.

FAZIT: System ist betriebsf�hig. Kritischste Aktion ist I-001 (chirurgischer 1-Zeilen-Fix).
       Ghost-Mirror (I-003) ist der gr��te offene Gap bevor Production-Release.
~~~

---

## LEGACY GATE GUARDRAIL (FINAL � UNVER�NDERLICH)

> **Vor JEDEM zuk�nftigen Vorschlag MUSS dieses Register gepr�ft werden.**
>
...
>
> BEGRABEN (vollst�ndige Liste):
> innerHTML direkt, richText-Flag, execCommand, new Date() f�r Timestamps,
> initCMABridge() setProperty-Batch, switchForm() setProperty-Call,
> Legacy-Key-Mapping, Pico-CSS-Full-Import, margin-top f�r DIN-Zonen,
> ID-basierte Felder-Selektoren (#f-subject etc.), JS-Klassen-Toggle f�r Sichtbarkeit,
> html[data-layout] als State-Tr�ger, Inline-onchange-Handler.
>
> CEMETERY IDs:
...
## � Dokument: 20_specify_chrome147_v1.2.0.md (Quelle: .brain)

---
tags: [Core-grade, Standard-2026, logic, blueprint, chrome-147]
status: cemented
version: 1.2.0
last_audit: 2026-03-23
id: BRAIN-020-SPEC
...
Das Ghost-Mirror Pattern trennt Datenerfassung (Souver�nit�t) von visueller Darstellung (�sthetik).
...
- **Phasen:**
  - `GREEN`: API aktiv.
  - `AMBER` (3 Strikes): Deaktivierung Autocomplete, R�ckfall auf Manual-Input.
  - `BLACK` (14 Strikes): Dead-Flag. Funktion verschwindet aus UI (Reaktivierung nur �ber Cockpit).
- **Heilung:** Ein einziger erfolgreicher Request setzt den Strike-Z�hler sofort auf 0 (Temporal API Validation).

## IV. LANDING ZONE (SIGNATURE PROTECTION)
Schutz der Unterschrift vor �berlauf auf die zweite Seite.
- **Mechanik:** CSS Scroll-State Queries pr�fen, ob `<din-signature>` die physische Blattkante (CMA-max-y) �berschreitet.
- **Reaktion:** Wenn Overflow detektiert wird -> automatischer CSS Page-Break f�r den gesamten Block.
- **Purity:** Kein JavaScript-Bounding-Box-Berechnen n�tig.


---


## � Dokument: 21_native_purity_sweep_v1.0.0.md (Quelle: .brain)

---
tags: [Core-grade, native-purity, future-proof, chrome-horizon]
...
Er ist ein pr�ziser Migrations-Schie�befehl:
Was heute stirbt, stirbt heute. Was morgen stirbt, wird heute markiert.
Kein Mittelweg. Keine Ausnahmen ohne explizite Begr�ndung.
...
| Datei | Zeile(n) | Aktuelles Pattern | Nativer Ersatz | Priorit�t | Kommentar |
|---|---|---|---|---|---|
| `app.js` | Z.32 | `initCMABridge(storedForm)` | ENTF�LLT � `css/:root` ist SSoT | ?? SOFORT | TOMB-B001. Import in Z.14 ebenfalls l�schen. |
| `app.js` | Z.38 | `formatDate(new Date(), fmt)` | `Temporal.Now.plainDate(Temporal.Now.timeZoneId())` | ?? SOFORT | TOMB-L001. `new Date()` f�r Timestamp-Generierung verboten. |
| `cma-bridge.js` | komplett | `initCMABridge()` 14x `setProperty` | CSS `:root {}` | ?? SOFORT | Nach L�schung: `cma-bridge.js` Datei komplett eliminieren wenn nur `switchForm()` �brig. |
| `cma-bridge.js` | `switchForm()` | `setProperty('--address-top', ...)` | Nur `paper.dataset.layout` setzen � CSS `project.overrides` �bernimmt Rest | ?? SOFORT | TOMB-B002. `--address-top` ist statisch. CSS leistet das bereits. |
...
| `ui.js` | `_bindTagInputs()` | `execCommand`-Block mit `querySelectorAll('[data-cmd]')` | ENTF�LLT � kein Ersatz n�tig | ?? SOFORT | TOMB-EXEC-001. plaintext-only ignoriert execCommand strukturell. |
...
| `ui.js` | `_safeSetHTML()` | `el.innerHTML = html` | ENTF�LLT � MANDATE-INJ | ?? SOFORT | TOMB-U002. Dead Code + Sicherheitsrisiko. |
...
| `ui.js` | `_bindProfileDialog()` | `document.getElementById('dialog-profile')?.hidePopover?.()` | `commandfor="dialog-profile" command="hide-popover"` am Save-Button | ?? N�CHSTE | BRAIN-015 B-1. Invoker Command. Chrome 133+. |
| `ui.js` | `_bindActions()` | `document.getElementById('btn-dev-reset')?.addEventListener('click', ...)` | `commandfor="dlg-confirm-reset" command="show-popover"` (wenn Confirm-Dialog existiert) | ?? SP�TER | Invoker Commands. Kein sofortiger Nutzen ohne Confirm-Dialog. |
| `ui.js` | `_toast()` | `setTimeout(() => el.remove(), 3200)` | **Partiell**: `animationend`-Event statt `setTimeout` | ?? SP�TER | Animation l�uft bereits 3.2s in CSS. `setTimeout(3200)` ist Duplikat. `el.addEventListener('animationend', () => el.remove())` ist robuster. Kein CSS-Ersatz m�glich (DOM-Cleanup). |
| `ui.js` | `_setStatus()` | `el.textContent = msg` bei jedem Boot/Action | CSS `content: attr(data-status)` auf `#statusbar` | ?? SP�TER | `statusbar.dataset.status = msg` ? CSS rendert. Eliminiert DOM-Write. |
| `devmode.js` | `_renderTagInspector()` | `new Date().toLocaleTimeString('de')` | `Temporal.Now.plainTimeISO(Temporal.Now.timeZoneId())` | ?? N�CHSTE | ANTI-016. `new Date()` f�r Timestamps verboten. Dritte Fundstelle nach `logic.js` und `app.js`. |
| `devmode.js` | `_bind5xClick()` | `body.dataset.toast = 'dev-unlocked'` + `setTimeout(() => delete body.dataset.toast, 3200)` | `@starting-style` f�r Entry-Animation bereits in CSS. `setTimeout` f�r Cleanup n�tig � **kein vollst�ndiger CSS-Ersatz**. | ?? SP�TER | CSS �bernimmt visuelle Animation. JS-Cleanup bleibt. |
| `devmode.js` | `_bindAkinatorTerminal()` | `btn.dataset.copied = 'true'` + `setTimeout(() => delete btn.dataset.copied, 1800)` | `@starting-style` f�r Button-Feedback-Animation | ?? SP�TER | `[data-copied="true"]` CSS-Regel existiert bereits in `sidebar.css`. Timing bleibt JS. |
...
| `model-blacklist.js` | `renderModelSelect()` | `el.innerHTML = ''` | `el.replaceChildren()` | ?? SP�TER | `replaceChildren()` ist semantisch klarer, sicherer, native DOM API. `innerHTML = ''` ist hier kein MANDATE-INJ-Versto� (kein User-Content), aber `replaceChildren()` ist die sauberere Alternative. |
| `state.js` | `save()` | `catch { /* quota exceeded � silent */ }` | `catch(e) { if (e instanceof DOMException && e.name === 'QuotaExceededError') { ... Priorit�ts-Recovery ... } }` | ?? N�CHSTE | G-002. BRAIN-009 v6.0.0 Teil V spezifiziert die Recovery-Logik. Typed catch ist erster Schritt. |
| `index.html` | L.1 | `<html data-layout="form-b">` | ENTF�LLT � vollst�ndig toter Code | ?? SOFORT | TOMB-HTML-002. Kein CSS-Selektor, kein JS liest `documentElement.dataset.layout`. Liegt immer auf `form-b` � l�gt nach Form-Wechsel. |
| `index.html` | Radios | `onchange="document.getElementById('paper').dataset.layout=...;localStorage.setItem(...)"` | Event-Delegation in `ui.js:_bindActions()` | ?? N�CHSTE | No-JS-Doctrine. Inline-Handler verletzen MANDATE-000 und erschweren Testing. |
| `index.html` | Checkbox | `onchange="document.getElementById('paper').dataset.guides=this.checked"` | Event-Delegation in `ui.js:_bindActions()` | ?? N�CHSTE | Identisches Problem. |
| `index.html` | 6x | `popovertarget` / `popovertargetaction` | `commandfor` / `command` | ?? N�CHSTE | BRAIN-015 B-2. Chrome 133+. Vollst�ndige Tabelle in `20_integrity_audit_v1.0.0.md` N-002. |

---

## 2. CSS ? HTML Attribute Migration Candidates

| Datei | Selektor / Regel | Aktuell | Vorgeschlagenes HTML + CSS | Priorit�t | Kommentar |
|---|---|---|---|---|---|
...
| `din5008-paper.css` | `:root {}` | 14x untypisierte Custom Properties | `@property --subject-top { syntax:"<length>"; initial-value:103.4mm; inherits:true; }` � 14 | ?? N�CHSTE | BRAIN-015 A-8. Chrome 85+. Typsicherheit, Animierbarkeit, Browser-Validierung. |
| `din5008-paper.css` | `@layer din.core` | Globale `din-*` Selektoren ohne Scope | `@scope (#paper) { @layer din.core { din-subject { ... } } }` | ?? N�CHSTE | BRAIN-015 A-3. Chrome 118+. Strukturelle Isolation. |
| `din5008-paper.css` | `:root {}` | Kein `interpolate-size` | `:root { interpolate-size: allow-keywords; }` | ?? N�CHSTE | BRAIN-015 A-5. Chrome 129+. Erm�glicht `height: auto` Transitions ohne JS. |
| `devmode.css` | `.toast` | `animation: toast-in-out 3.2s forwards` � Entry-Phase via Keyframe 0%�10% | `@starting-style { .toast { opacity: 0; transform: translateY(12px); } }` + Transition statt Keyframe f�r Entry | ?? N�CHSTE | BRAIN-015 A-4. Chrome 117+. Saubere Trennung Entry/Exit-Animation. |
| `devmode.css` | `#toast-dev` | `bottom: -80px` default + `transition: bottom 0.3s` + `animation: toast-stay 3.2s` | `@starting-style { #toast-dev { opacity: 0; translate: 0 80px; } }` | ?? N�CHSTE | Doppelt-Animation: einmal `transition`, einmal `@keyframes`. Widerspr�chlich. `@starting-style` bereinigt das. |
| `devmode.css` | `#iban-ghost` | `position: absolute; inset: 0` auf Wrapper | Mit CSS Anchor Positioning: `#p-iban { anchor-name: --iban-field; }` ? Ghost positioniert via `position-anchor` | ?? SP�TER | Chrome 125+. Elegant aber kein dringender Defekt. Heutiger Stand funktioniert korrekt. |
| `sidebar.css` | `.sidebar-btn:hover` | `background: #333` (hardcoded Duplicate) | `oklch(from var(--sidebar-btn-bg) calc(l + 0.05) c h)` | ?? N�CHSTE | BRAIN-015 D-1. Relative Color Syntax. Chrome 119+. Alle Hover/Active-Varianten mathematisch. |
| `sidebar.css` | `.sidebar-btn.primary:hover` | `background: #2a5a8a` (hardcoded) | `oklch(from var(--sidebar-btn-primary) calc(l + 0.05) c h)` | ?? N�CHSTE | Identisch. |
| `sidebar.css` | `#akinator-output` | `rows="10"` Hardcoded + kein Resize-CSS | `textarea { field-sizing: content; min-height: 3lh; max-height: 60vh; }` | ?? N�CHSTE | BRAIN-015 A-6. Chrome 123+. `rows` Attribut entfernen. |
| `din5008-paper.css` | `project.overrides` | `[data-placeholder]:empty::before { content: attr(data-placeholder); }` | Bereits korrekt � kein Handlungsbedarf | ? PERFEKT | Das ist die Vorlage f�r das Pattern. Kein Fix n�tig. |
| `din5008-paper.css` | `din-salutation[data-gender="..."]` | `::before` rendert Placeholder | Bereits korrekt � CSS ohne JS | ? PERFEKT | No-JS-Doctrine vollst�ndig erf�llt. |
| `devmode.css` | `body[data-devmode="true"] .dev-only` | `display: flex` | Korrekt � data-Attribut-gesteuerter Toggle | ? PERFEKT | ADR-003 erf�llt. |
...
| `din5008-paper.css` | `@media print { ... !important }` | `!important` in Print-Reset | Durch `@layer` �berfl�ssig � Print lebt au�erhalb der Layer | ?? SP�TER | Die `!important` in Print-Rules sind technisch korrekt (Print �berschreibt Layer), aber mit `@layer`-Architektur k�nnten sie via Spezifit�t gel�st werden. Kein dringender Fix. |
...
Signalisieren zuk�nftigen Agenten: �Hier wartet ein Upgrade."
...
**Marker heute setzen in:** `css/din5008-paper.css` � Kommentar �ber `din-subject` Regel:
~~~
...
~~~
**Marker heute setzen in:** `index.html` � �ber jedem `<din-*>` Tag:
~~~html
...
**Marker heute setzen in:** `css/din5008-paper.css` �ber `:root {}`:
~~~css
...
### FP-003 | CSS `if()` � Inline-Konditionals f�r Sidebar-Theming
**Ziel-Feature:** CSS `if()` Inline-Funktion � `color: if(style(--scheme: dark): white; else: black)`
**Chrome-Horizon:** ~148 (Nightly 2025 verf�gbar), GA ~150
**Heute relevanter Code:** `css/sidebar.css` � manuelle Dark-Theme Farbwerte
**Marker heute setzen in:** `css/sidebar.css` �ber Sidebar-Farb-Variablen:
~~~css
...
### FP-004 | `text-wrap: balance` f�r `din-subject`
...
F�r zuk�nftigen mehrzeiligen Modus (SPEC-Kandidat):
~~~css
...
### FP-005 | `text-wrap: pretty` f�r `din-body-mirror`
...
**Marker heute setzen in:** Kommentar in `css/din5008-paper.css` �ber `din-body` Regel:
~~~css
/* FUTURE-PROOF: text-wrap: pretty � Chrome 117+ stabil.
   ...
   Verhindert typografisch unsch�ne letzte Zeilen (Hurenkinder). */
~~~
...
**Marker heute setzen in:** `css/sidebar.css` �ber `#sidebar-left`:
~~~css
...
**Ziel-Feature:** `@container style(--theme: dark)` � Chrome 111+ f�r Custom Properties (bereits stable!)
**Chrome-Stand: JETZT EINSETZBAR** f�r Custom Property Style Queries
...
### FP-008 | `position-try-fallbacks` f�r Anchor-Tooltips
...
**Marker heute setzen in:** Kommentar in zuk�nftiger Tooltip-Implementierung:
~~~css
/* FUTURE-PROOF: position-try-fallbacks Chrome 125+ stabil.
   Alle din-* Tooltips m�ssen dieses Pattern tragen:
   .din-hint {
     position-anchor: --din-[key];
     position-area: inline-end center;
     position-try-fallbacks: flip-inline, flip-block, flip-inline flip-block;
   }
   Verhindert Viewport-Overflow automatisch. Kein JS n�tig. */
~~~
...
**Marker heute setzen in:** Kommentar in `index.html` �ber Editor-Toolbar:
~~~html
<!-- FUTURE-PROOF: interestfor + popover=hint Chrome 135+ stabil.
     F�r alle 11 din-* Tooltips (IMR Sektion L):
...
[ ] D-004  ui.js: entry.richText Ternaries (3x) ? direkter _safeSet() Aufruf
           /* DEAD: See TOMB-L008 */
[ ] D-005  index.html: data-layout="form-b" von <html> entfernen
           <!-- REMOVED: See TOMB-HTML-002 � vollst�ndig toter Code -->
[ ] N-001  index.html + ui.js: btn-import-trigger ? label for="file-import"
[ ] N-006  css/din5008-paper.css: @layer latex.base hinzuf�gen (erste Zeile)
[ ] app.js Z.38: formatDate(new Date()) MARKER setzen:
           /* NATIVE REPLACE ME � See TOMB-L001 / BRAIN-018: Temporal.Now.plainDate(timeZoneId) */
~~~

### ?? N�CHSTE RUNDE � Spec-Debt + Chrome-Native Umstellungen
...
[ ] N-006b css: @property f�r alle 14 CMA-Variablen
[ ] N-007  css: @scope (#paper) um @layer din.core
[ ] N-007b css: interpolate-size: allow-keywords in :root
[ ] N-008  css/sidebar.css: field-sizing: content f�r #akinator-output
[ ] CSS    css/sidebar.css: Relative Color Syntax f�r Hover-Farben
[ ] CSS    devmode.css: @starting-style f�r Toast Entry-Animation
...
~~~
[ ] FP-001 css/din5008-paper.css: Marker f�r Typed attr() ~Chrome 150
[ ] FP-002 css/din5008-paper.css: Marker f�r CSS @function ~Chrome 155
[ ] FP-003 css/sidebar.css: Marker f�r CSS if() ~Chrome 148�150
[ ] FP-004 css/din5008-paper.css: Marker f�r text-wrap: balance (din-subject)
[ ] FP-005 css/din5008-paper.css: Marker f�r text-wrap: pretty (din-body-mirror)
[ ] FP-006 css/sidebar.css: Marker f�r @container Responsive-Sidebar
[ ] FP-007 css/devmode.css: Marker f�r CSS Style Queries
[ ] FP-008 Tooltip-Implementierung: Marker f�r position-try-fallbacks
[ ] FP-009 index.html: Marker f�r interestfor + popover=hint
~~~

### ?? EIGENE SPECS � Gro�e Implementierungs-Projekte

~~~
[ ] I-003+I-004  din-body-mirror im DOM + break-inside (KRITISCHER GAP)
                 Solange nicht implementiert: Print ist nicht DIN-konform f�r Markdown
...
## Besondere Funde � Nicht in Audit-Vorg�nger dokumentiert

### NEU-001 | `devmode.js` � Dritte `new Date()` Fundstelle | ANTI-016

`_renderTagInspector()` Z.~67: `new Date().toLocaleTimeString('de')`

Dritte ANTI-016-Verletzung. `toLocaleTimeString('de')` ist locale-abh�ngig UND
gibt keinen deterministischen String zur�ck. Wenn Temporal vollst�ndig migriert ist:
...
Technisch kein MANDATE-INJ-Versto� (kein User-Content, nur UI-Reset).
...
`#toast-dev` hat sowohl `transition: bottom 0.3s ease` (f�r Slide-in)
als auch `animation: toast-stay 3.2s forwards` (f�r Stay+Slide-out).
...
Saubere L�sung: `@starting-style` f�r Entry + `@keyframes` nur f�r Exit.
Kein `transition` auf `bottom` mehr n�tig.
...
~~~
Auditor:  Lead Native Purity Architect (Core Grade)
Datum:    2026-03-21
Methode:  Vollst�ndiger Zeilenscan aller JS/CSS/HTML Dateien
          Context7 f�r Chrome 150�160+ Horizon-Features
Neu entdeckt: 4 Befunde die in v19/v20 fehlten (NEU-001 bis NEU-004)

Fazit:
Das System ist in einem besseren Zustand als sein Code-Debt vermuten l�sst.
Die Kern-Architektur (IMR, @layer, ADR-008/009) ist sauber.
Der Debt ist konzentriert auf 5 Dead-Code-Bl�cke (D-001 bis D-005),
eine ADR-008-Verletzung im DOM (I-001), und ausstehende Chrome-146+ Migrationen.

Kritischste Aktion: I-001 � ein Zeichen-�nderung im Live-DOM.
...
data-cma-* Attribute bereit f�r Typed attr(). @layer-Architektur bereit f�r latex.base.
IMR Sektion L bereit f�r Anchor Positioning. 9 Horizon-Marker identifiziert.
~~~
...
## � Dokument: 24_Standard_level_up_chrome147_v1.0.0.md (Quelle: .brain)

# Standard Level-Up (Chrome 147 Baseline)
# Version: 1.0.0 | Date: 2026-03-21
# Core Grade Standard | Brain-First & Spec-Driven

## Executive Summary
Das DIN-BriefNEO Projekt wurde vollst�ndig auf die **Google Chrome 147.0+ Baseline** migriert. Jegliche Legacy-Workarounds, Polyfills (`@supports`) und veraltete JS-APIs wurden restlos eliminiert (No Legacy, No Backport, No Mercy). 

Die Architektur nutzt ab sofort exklusiv:
- **Temporal API** anstelle von fehleranf�lligen `new Date()` Objekten.
- **Native Invokers** (`commandfor` / `command`) f�r Modals und Toasts (ersetzt `popovertarget`).
- **CSS `@property`** f�r die strenge Typisierung der 14 CMA-Variablen im CSS (Length).
- **CSS `@scope`** zur Kapselung des DIN-Papiers.
- **Scoped View Transitions** (`Element.startViewTransition()`) Vorbereitung.
- **Future-Proof Marker** f�r Chrome 148/149-Features (`@function`, `if()`, `position-try`, `Interest Invoker`).

## Aktualisierte TOMB-Registry (Zementiert)
- **TOMB-LEGACY-001**: `new Date()` (au�er `parseDate` / User Input) ersetzt durch `Temporal` API (`temporal-utils.js`).
- **TOMB-LEGACY-002**: `popovertarget` / `popovertargetaction` ? `commandfor` / `command`.
- **TOMB-LEGACY-003**: manuelle `addEventListener('click')` f�r Dialoge/Toasts gel�scht (jetzt Native Invokers).
- **TOMB-LEGACY-004**: JS `.click()` Trigger ? native `<label for="file-import">` in der Sidebar.
- **TOMB-LEGACY-005**: `execCommand` + `richText` Code-Bl�cke restlos entfernt (Ghost-Mirror Transition).
- **TOMB-LEGACY-006**: `@supports` f�r moderne Features aus `din5008-paper.css` entfernt.
- **TOMB-LEGACY-007**: `data-layout` Attribut auf `<html>` und `#paper` Container gel�scht.
- **TOMB-LEGACY-008**: Datei `js/core/cma-bridge.js` + Import `initCMABridge()` restlos gel�scht (vollst�ndig CSS-nativ).
- **TOMB-LEGACY-009**: Manuell via JS gesetzte `--cma-*` Properties entfernt (jetzt CSS `@property` Typed Variables).

## Vollst�ndige Diff-�bersicht
| Datei | �nderung |
| --- | --- |
...
| `js/logic/logic.js` | `new Date()` f�r `todayISO`/`formatDate` entfernt und auf Temporal umgebogen. |
...
| `index.html` | Migration von `popovertarget` zu `commandfor`+`command`, `<label>` f�r Import, `data-layout` entfernt. |
| `js/ui/ui.js` | `_bindActions` komplett gel�scht, `execCommand`-Block entfernt, Event-Delegation optimiert. |
| `css/din5008-paper.css` | 22 CMA-Variablen via `@property` typisiert. `@layer latex.base` erg�nzt. `@scope (#paper)` vorbereitet. Future-Proofing Kommentare eingef�gt. |
| `js/core/cma-bridge.js` | **GEL�SCHT** (TOMB-LEGACY-008). |
...
- [x] Kein `new Date()` (au�er Parsing), kein `execCommand`, kein `popovertarget`, kein `@supports`, kein `cma-bridge.js` mehr.
...
- [x] Neue `.brain/24_Standard_level_up_chrome147_v1.0.0.md` Datei existiert.
- [x] Das Projekt ist ab sofort Chrome-147-native, spec-driven und Core-grade.
...
**RULE ZERO**: Bestehende .brain-Dateien nie �berschreiben ? nur neue _vX.Y.Z.md anlegen.

**DIRECTIVE**:
- Neue unverr�ckbare Baseline: Google Chrome 147.0+ (Beta aktiv, Stable 07.04.2026)
- Alles, was 147 nativ kann ? SOFORT nutzen (KEIN JS wo m�glich)
...
## � Dokument: 25_environment_integrity_v1.0.0.md (Quelle: .brain)

# ?? THE ULTIMATIVE Standard ENGINE AUDIT (V5 - 2026)
# STATUS: IMMUTABLE EVIDENCE GEM
# Erstellt: 23.03.2026 | User-Agent: Chrome/146.0.0.0 (Windows)

Dieser Bericht ist die zementierte Wahrheit �ber die F�higkeiten der Ziel-Engine.
...
| :--- | :--- | :--- |
| CSS :has() Selector | ? | Unverzichtbar f�r No-JS UI |
| CSS if() Function | ? | **SSoT f�r Layout-Logik** |
| CSS mix() Function | ? | Flie�ende �berg�nge |
| Advanced attr() (Types) | ? | **Direkt-Mapping von CMA** |
| View Transition Triggers | ? | Deklarative Animationen |
| CSS Custom States (:state) | ? | Kapselung von UI-Zust�nden |
| CSS @scope | ? | Isolation von Styles |

## 2. LAYOUT ENGINE (Core Grade Precision)
| Feature | Status | Notiz |
| :--- | :--- | :--- |
...
| Container Scroll State | ? | Basis f�r Mehrseitigkeit |
...
| Font Metrics API | ? | Pr�zise Zeilenbruch-Vorhersage |
...
| Trusted Types API | ? | System-weite XSS-Pr�vention |
...
## ??? ARCHITECT'S STRATEGY (Abgeleitet am 23.03.2026)

### ?? Die "Go-Live" Mandate (Sofortige JS-Eliminierung):
1. **cma-bridge.js L�SCHEN:** `CSS if()` und `attr(data-top mm)` funktionieren. Das JS-Binding der Ma�zahlen wird entfernt.
2. **Toolbar-JS L�SCHEN:** `CSS Anchor Positioning` funktioniert. Die Position wird rein deklarativ im CSS verankert.
3. **JS-Resize L�SCHEN:** `field-sizing: content` funktioniert. Kein Observer mehr f�r die H�he des Bodys n�tig.
4. **contenteditable ERSETZEN:** Die `EditContext API` und der `Sanitizer (setHTML)` sind live. Der Ghost-Mirror wird sicher.
5. **Druck-JS L�SCHEN:** `@page` Margin Boxes und Paginierung (`break-inside`) funktionieren nativ.

### ?? Die "Resilience" Workarounds (JS-Fallbacks bleiben):
- Da `:has()` in diesem speziellen Build unerwartet auf `?` steht (m�glicherweise hinter einem Flag oder aufgrund des speziellen Selektor-Tests), m�ssen wir pr�fen, ob wir einfache Klassen-Toggles (`.is-formal`) f�r das Layout behalten m�ssen.
- `@scroll-state` fehlt. Der `IntersectionObserver` bleibt f�r das Paginierungs-Tracking (BRAIN-029) erhalten.


---


## � Dokument: 26_ghost_mirror_sanitized_v1.0.0.md (Quelle: .brain)

# Meilenstein: Ghost-Mirror & Native Security (Chrome 147/148)
# Version: 1.0.0 | Datum: 21.03.2026
# Status: ZEMENTIERT | Core Grade Standard

## 1. Executive Summary
In dieser Session wurde das DIN-BriefNEO System auf die n�chste Evolutionsstufe gehoben. Wir haben die Trennung von **Datenhaltung (Plaintext)** und **visueller Darstellung (Mirror)** durch das Ghost-Mirror Pattern vollzogen. Dabei wurden die neuesten nativen Features von Chrome 147 (Sanitizer API, Scroll-State) und Chrome 148 (CSS if-Logic) produktiv geschaltet.

## 2. Technologische Durchbr�che
...
- **L�sung**: Umstellung auf `contenteditable="plaintext-only"`.
- **Visualisierung**: Ein `<din-body-mirror>` spiegelt den Inhalt formatiert wider.
- **Security**: Die Synchronisation erfolgt �ber die **Chrome 147 Sanitizer API** (`Element.setHTML()`). Der Browser garantiert auf Engine-Ebene, dass kein schadhafter Code in die Darstellung gelangt. Jeglicher Legacy-Sanitizer-Code wurde ersatzlos gel�scht.
...
- **Overflow-W�chter**: Ein **Zero-JS Indikator** �berwacht mittels `scroll-state(scrollable: true)` die Textl�nge und warnt bei �berlauf auf die zweite Seite � ohne eine einzige Zeile JavaScript-Eventlistener.

### 2.3 Finaler Temporal-Sieg (TOMB-LEGACY-001)
- Die Funktion `parseDate` in `logic.js` wurde vollst�ndig auf die **Temporal API** migriert.
- Es existiert nun **keine Instanz** von `new Date()` mehr im gesamten Projekt-Core. Das System ist mathematisch deterministisch.

## 3. Durchgef�hrte L�schungen (Legacy Purge)
- **TOMB-L008**: `richText`-Flags und deren Logik-Pfade in `ui.js` vollst�ndig entfernt.
...
- [x] Layout-Toggle arbeitet �ber native CSS-Logik (`if`).
- [x] Overflow-Warnung erfolgt rein via CSS (`scroll-state`).
- [x] Temporal API ist die einzige Quelle f�r Zeit/Datum.
- [x] Readiness-Report v25 best�tigt 11/14 Features als ? READY.

---
**Dieses Projekt operiert nun am Limit des technisch M�glichen der Web-Plattform 2026.**


---


## � Dokument: 27_cemetery_consolidation_v1.0.0.md (Quelle: .brain)

# Mission Report: Cemetery Consolidation & Native Victory
# Version: 1.0.0 | Datum: 21.03.2026
# Status: ARCHIVIERT | Core Grade Standard

## 1. Executive Summary
Diese Session markiert den endg�ltigen �bergang von einer JS-gesteuerten Anwendung zu einer **Browser-nativen Engine**. Wir haben das Prinzip der "Deklarativen Souver�nit�t" durchgesetzt. Das System nutzt nun physische Zust�nde (Radio/Checkbox) und native Logik (`:has`, `if`), anstatt diese m�hsam �ber JavaScript zu simulieren.

## 2. Die Liste der Begrabenen (The Great Purge)

### 2.1 JavaScript UI-Logic
- **Status**: **TOT**. 
- Alle Funktionen, die Attribute wie `data-layout` oder `data-guides` gesetzt haben, wurden gel�scht.
- Alle `onchange` Handler in der `index.html` wurden restlos gerodet.
- **Nutzen**: Null Latenz bei der Interaktion, da kein JS-Parsing-Zyklus ben�tigt wird.
...
- **Nutzen**: Der Browser garantiert die fl�ssigste Animation auf Hardware-Ebene.
...
## 3. Die neue Ordnung (Core Grade Standard)
1.  **HTML**: H�lt den Status (SSoT) �ber native Formularelemente.
2.  **CSS**: Enth�lt die gesamte Logik f�r Geometrie, Sichtbarkeit und Animation.
3.  **JS**: Dient ausschlie�lich als Daten-IO f�r Briefinhalte (IMR).
...
## � Dokument: 29_specify_multipage_v1.0.0.md (Quelle: .brain)

---
tags: [Core-grade, Standard-2026, logic, multipage, css-147]
status: cemented
version: 1.0.0
last_audit: 2026-03-23
id: BRAIN-029-SPEC
...
## I. PROBLEMSTELLUNG
Ein DIN-Brief kann �ber die physische Grenze von Seite 1 (DIN A4 minus R�nder) hinauswachsen. Die Herausforderung besteht darin, diesen �berlauf OHNE JavaScript-Layout-Berechnungen (Jitter-Gefahr) zu erkennen und das UI f�r Folgeseiten anzupassen.

## II. DEKLARATIVE �BERLAUF-ERKENNUNG
...
- **Logic:** Ein unsichtbarer Detektor am Ende von Seite 1 pr�ft den `scroll-state`.
...
Wenn der �berlauf detektiert wird, greifen folgende `@layer project.overrides` Regeln:
...
### �2 Fu�zeilen-Integrit�t
- Die Fu�zeile bleibt station�r am unteren Blattrand (Fixed Position im Druck-Kontext).
- CMA-Koordinate: `bottom: 15mm`.

### �3 Margins & Flow
- Der obere Rand von Seite 2 wird auf `top: 20mm` reduziert (keine Anschriftzone mehr n�tig).
- Der Textfluss wird nahtlos fortgesetzt, wobei `orphans: 3` und `widows: 3` eine typografische Mindestqualit�t garantieren.

## IV. VALIDIERUNG
- **Test-Szenario:** Einf�gen eines 5000-Zeichen-Textes in den Body.
...
## � Dokument: 33_native_css_supremacy_v1.0.0.md (Quelle: .brain)

# 33 � Native CSS Supremacy v1.0.0

## I. BEFUND
Natives CSS hat durch Features wie Custom Properties (`--var`) und Native Nesting die Kernfunktionen von Pr�prozessoren (SASS, LESS) vollst�ndig absorbiert.
...
- **Fehlervermeidung**: Keine k�nstlich aufgebl�hten Selektor-Ketten durch "Flattening" in SASS-Kompilaten.

## III. STATUS
- **G�ltig ab**: Chrome Baseline 147
...
## � Dokument: 34_css_state_machine_logic_v1.0.0.md (Quelle: .brain)

# 34 � CSS State-Machine Logic v1.0.0

## I. BEFUND
Die Einf�hrung von `@layer`, `@scope`, Container Queries und des `:has()`-Selektors transformiert CSS in eine Turing-nahe State-Machine.

## II. ARCHITEKTONISCHE BEDEUTUNG
- **Modularit�t via `@scope`**: Isolation von DOM-Fragmenten ohne BEM-Methodik oder SASS-String-Konkatenation.
- **Native Logik via `:has()`**: Komplexe "Wenn-Dann"-Bedingungen (z.B. Fehler-Visualisierung im �bergeordneten Container) direkt im CSS ausf�hrbar.
- **Dynamik**: Ersetzung von 80-90% der SASS-Funktionalit�t durch native, browser-interne Logik-Ebenen.
...
## � Dokument: 35_native_css_math_engine_v1.0.0.md (Quelle: .brain)

# 35 � Native CSS Math-Engine v1.0.0

## I. BEFUND
CSS unterst�tzt native mathematische Operationen (`calc()`, `clamp()`, `min()`, `max()`) sowie trigonometrische Funktionen (`sin()`, `tan()`, `cos()`).

## II. ARCHITEKTONISCHE BEDEUTUNG
- **Millimeter-Pr�zision (DIN 5008)**: Dynamische Berechnung von Abst�nden zur Laufzeit, basierend auf der physischen Blattgr��e.
- **Laufzeit-Evaluierung**: Im Gegensatz zu SASS (statische Berechnung zur Build-Zeit) reagiert die native Engine dynamisch auf Viewport-�nderungen und Resize-Ereignisse.
- **Hardwarebeschleunigung**: Direkte Ausf�hrung mathematischer Operationen durch die Browser-Engine.
...
## � Dokument: 40_native_invoker_dispatch_v1.0.0.md (Quelle: .brain)

# 40 � Native Invoker Dispatch v1.0.0

## I. BEFUND
Die Invoker Commands API (Chrome 135+) und Interest Invoker API delegieren UI-Zustandsmanagement (Klicks, Hover-Effekte, �ffnen/Schlie�en von Popovers) direkt an die C++-Dispatcher der Blink-Engine.

## II. ARCHITEKTONISCHE BEDEUTUNG
- **Logic-Lite UI**: JS-Event-Listener (`click`, `mouseenter`) f�r UI-Zust�nde sind obsolet.
- **Performance**: Vermeidung von Main-Thread-Jumps zwischen C++ und V8-Engine f�r reine UI-Aktionen.
- **Trennung der Belange**: HTML steuert die UI-Wege (`commandfor`, `interestfor`), JS feuert nur noch Dom�nenlogik (I/O, State) als Reaktion auf das `command` Event.
...
## � Dokument: 41_css_kinetic_state_machine_v1.0.0.md (Quelle: .brain)
...
## II. ARCHITEKTONISCHE BEDEUTUNG
- **Native Intrinsische Animationen**: `calc-size()` erm�glicht nahtlose �berg�nge zu und von intrinsischen Werten (`auto`, `max-content`) ohne JavaScript-H�henberechnungen.
- **Scroll-Observer Obsolet**: Erkennung von Text�berlauf (`scrollable`) oder Sticky-Zust�nden wandert komplett in die C++-Layout-Pipeline.
- **Jank-Frei**: 120fps Rendering f�r kinetische UI-Elemente, da der V8-Main-Thread umgangen wird.
...
## � Dokument: 42_native_idle_detection_v1.0.0.md (Quelle: .brain)

# 42 � Native Idle Detection v1.0.0

## I. BEFUND
Die Native Idle Detection API (Chrome 94+) erkennt Inaktivit�t direkt auf Betriebssystemebene.

## II. ARCHITEKTONISCHE BEDEUTUNG
- **Ersatz f�r Debouncing**: JS-basierte Timer (`setTimeout`, `setInterval`) zur Erkennung von "Schreibpausen" f�r Autosaves belasten den Main-Thread und sind verboten (TOMB-L008).
- **Ressourceneffizienz**: Der OPFS Background-Sync wird exakt dann ausgel�st, wenn das OS Hardware-Leerlauf meldet.
- **Verl�sslichkeit**: OS-Hooks (Maus, Tastatur) sind pr�ziser als isolierte DOM-Events.
...
## � Dokument: 43_css_advanced_logic_v1.0.0.md (Quelle: .brain)

# 43 � CSS Advanced Logic & Typed attr() v1.0.0

## I. BEFUND
Typed `attr()` und `CSS if()` verlagern dom�nenbasierte Design-Logik in die C++-Engine.
...
- **Native if/else**: CSS berechnet Abst�nde dynamisch basierend auf Variablen und Attributen.
...
## � Dokument: 47_specify_flight_recorder_v1.0.0.md (Quelle: .brain)

---
tags: [Core-grade, Standard-2026, flight-recorder, ki-native, spec]
status: cemented
version: 1.0.0
last_audit: 2026-03-23
id: BRAIN-047-SPEC
...
## I. INTENT
Spezifikation der Daten-Schnittstelle zwischen dem BriefNEO-Client und dem Black Box Decoder (KI-Agent). Das Ziel ist die Bereitstellung eines 100% konsistenten Fehler-Kontextes f�r deterministische Fehlerkorrektur.
...
2. **`RESET_LOCAL_STORAGE_PHASE`**: Gezieltes Zur�cksetzen der Resilienz-Phasen.
3. **`NOTIFY_USER_CMA_CRITICAL`**: Benutzerhinweis bei physischer Unm�glichkeit (z.B. Adressfeld zu klein f�r Input).

## IV. VALIDIERUNG
- **Check:** Stimmen die Schl�ssel der Payload mit der IMR 2.5 �berein?
...
## � Dokument: 55_Standard_readiness_matrix_v1.0.0.md (Quelle: .brain)

# ?? Standard READINESS MATRIX V1.0.0
# Baseline: Chrome 147.0+ (Standard 2026 Engine)
# Status: Core Grade Verified

Dieses Dokument dient als Single Source of Truth (SSoT) f�r alle genutzten und verf�gbaren modernen Browser-APIs im DIN-BriefNEO Projekt. Es katalogisiert die technologische �berlegenheit der "Blink-Direct" Architektur.
...
| **Temporal API** | Chrome 146+ | Pr�zise, deterministische Zeitrechnung ohne Date-Hacks. | ? AKTIV |
...
| **Highlight API** | Chrome 105+ | Syntax-Coloring f�r Markdown-Marker ohne DOM-Payload. | ? AKTIV |
| **contrast-color()** | Chrome 147+ | Native Kontrast-Berechnung f�r UI-Elemente. | ? BEREIT |
| **border-shape** | Chrome 147+ | Komplexe, nicht-rechteckige UI-Borders. | ? BEREIT |
| **@container scroll-state** | Chrome 147+ | �berlauferkennung f�r WYSIWYG ohne JS-Messung. | ? AKTIV |
...
1.  **Zero-innerHTML**: Jede DOM-Manipulation erfolgt �ber `setHTML` oder `textContent`.
...
## � Dokument: 55_Standard_readiness_matrix_v1.1.0.md (Quelle: .brain)

# ?? Standard READINESS MATRIX V1.1.0
# Baseline: Chrome 147.0+ (Standard 2026 Engine)
# Last Scan: 2026-03-25 | Status: Core Grade (With Exceptions)

Dieses Dokument katalogisiert die technologische �berlegenheit der "Blink-Direct" Architektur basierend auf physischen Engine-Audits.

---

## ?? KERN-APIs (Pfeiler 1-9)

| API | System-Nutzen | Audit-Status | Integrit�t |
| :--- | :--- | :--- | :--- |
| **EditContext API** | Entkopplung von Input & DOM. | ? AKTIV | ?? VERIFIZIERT |
| **Temporal API** | Pr�zise, deterministische Zeitrechnung. | ? AKTIV | ?? VERIFIZIERT |
| **Native Sanitizer** | Sicherer Ghost-Mirror Export via `setHTML()`. | ? AKTIV | ?? VERIFIZIERT |
| **Math.sumPrecise** | Fehlerfreie finanzielle Kalkulationen. | ? AKTIV | ?? INSTABIL� |
...
> � **Note on Math.sumPrecise**: Der Test `[0.1, 0.2] === 0.3` schlug im Scan fehl. Die API ist vorhanden, aber die Pr�zisions-Garantie der Beta-Engine ist f�r diesen spezifischen Edge-Case noch instabil.

---

## ?? CSS SUPREMACY (Blink-Direct Rendering)

| Feature | System-Nutzen | Audit-Status | Integrit�t |
| :--- | :--- | :--- | :--- |
...
| **border-shape** | Komplexe UI-Borders. | ? FEHLT� | ?? INSTABIL |
| **@container scroll-state** | WYSIWYG �berlauferkennung. | ? AKTIV | ?? VERIFIZIERT |

> � **Note on border-shape**: Feature in der aktuellen Engine-Konfiguration nicht verf�gbar (ggf. `chrome://flags`).

---

## ??? ZUS�TZLICHE VERIFIZIERTE SCHNITTSTELLEN

- **BroadcastChannel**: Real-Time Tab Sync (Verifiziert).
- **Compression Streams**: Native Export-Kompression (Verifiziert).
- **Intl.Segmenter**: Native Text-Analyse f�r Markdown (Verifiziert).
- **Device Memory API**: Memory-Aware Rendering (Verifiziert).
- **EyeDropper API**: Native Farbpicker (Verifiziert).

---

## ??? SYSTEM-AUDIT TELEMETRIE (Incident-ID: b726e6ae-...)
Der Scan vom 25.03.2026 best�tigt eine **95% Native Compliance**. Die Architektur ist f�r den Einsatz der verifizierten "??" APIs freigegeben.
...
## � Dokument: constitution_v1.0.0.md (Quelle: .brain)
...
## RULE ZERO
Bestehende .brain-Dateien nie �berschreiben ? nur neue _vX.Y.Z.md anlegen.
...
- **TOMB-LEGACY-001**: `new Date()` (au�er `parseDate` / User Input) ersetzt durch `Temporal` API
- **TOMB-LEGACY-002**: `popovertarget` / `popovertargetaction` ? `commandfor` + `command`
- **TOMB-LEGACY-003**: manuelle `addEventListener('click')` f�r Dialoge/Toasts
- **TOMB-LEGACY-004**: JS `.click()` Trigger ? native `<label for="">`
- **TOMB-LEGACY-005**: `execCommand` + `richText` entfernt
- **TOMB-LEGACY-006**: `@supports` f�r moderne Features entfernt
- **TOMB-LEGACY-007**: `data-layout` auf `<html>` entfernt
- **TOMB-LEGACY-008**: `cma-bridge.js` + `switchForm()` + `initCMABridge()` gel�scht
- **TOMB-LEGACY-009**: manuell gesetzte `--cma-*` Properties via JS gel�scht
...
## � Dokument: LEGACY_KNOWLEDGE_DUMP.md (Quelle: archive)

# ?? LEGACY_KNOWLEDGE_DUMP.md - Erhaltenes Wissen aus alten GEMINI.md Files
# Status: ARCHIVED | Stand: M�rz 2026

Dieses Dokument sichert spezifische Architektur-Details und Workflows, die aus den veralteten GEMINI.md Dateien (V9 Standard) extrahiert wurden, um den "Knowledge Shredder Protocol" zu erf�llen und Wissensverlust zu vermeiden.

## ??? Architektur-Details (Legacy V9)
...
- **Data Schema:** "No Ghost Data" Policy - Versteckte Felder in `storage.js` / `StateManager` m�ssen bei Save/Load Operationen erhalten bleiben, auch wenn sie nicht in der UI sichtbar sind.
- **Visual Freeze:** Die CSS-Geometrie in `css/din5008-paper.css` ist f�r Kern-Ma�e unantastbar (Zero Pixel Shift).
- **Desktop First:** Mobile Layouts sind gem�� ADR-019 explizit deaktiviert.
...
- **Rule #1:** Niemals Code direkt �ndern. Erst das Verzeichnis `.brain/` (Logic, Specs, Roadmap) aktualisieren.
- **Traceability:** Nutzung von Traceability IDs (z.B. `[DIN-...]`) bei jeder Code-�nderung.
- **Vanilla Only:** Striktes Verbot von Frameworks (React, Vue, etc.), jQuery, Lodash oder jspdf.

---
*Extrahiert am 26. M�rz 2026 zur Integration in die Standard v14 (DIN-BriefNEO) SSoT.*


---


## � Dokument: TEMPLATE.md (Quelle: research)
...
## � Dokument: specify.md (Quelle: 059-persistence-pwa)

---
id: SPEC-059
title: Persistence & Desktop PWA Readiness
tags: [persistence, pwa, auto-save, Core-grade]
status: cemented
version: 1.0.0
...
## 1. Zielsetzung
Maximale Datensicherheit durch intelligente Hintergrund-Speicherung und Transformation der Web-Oberfl�che in eine installierbare Desktop-Anwendung (PWA).
...
- Der Speicherprozess DARF NICHT den Schreibfluss st�ren (keine Latenz).
- Die Speicherung erfolgt ereignisbasiert: Entweder bei Fokusverlust oder nach einer definierten Unt�tigkeit w�hrend des Tippens (Denkpause).

### FR-002: Desktop App Experience (Installability)
- Die Anwendung MUSS als eigenst�ndige Desktop-App installierbar sein.
- Beim Start als App DARF KEINE Browser-UI (Adresszeile, Tabs) sichtbar sein.
- Die App MUSS offline-f�hig sein (Basis-Ressourcen gecached).
...
## � Dokument: specify.md (Quelle: 058-native-sanitizer)

---
id: SPEC-058
title: Startup Performance & UI Polish
tags: [performance, ux, polish, Core-grade]
status: cemented
version: 1.0.0
...
- Die gew�hlte Layout-Variante (Form A/B) MUSS vom Browser geladen sein, bevor das erste Element auf den Bildschirm gezeichnet wird.
- Ziel: Zero-Jank beim initialen Seitenaufruf.

### FR-002: Hochwertige Benachrichtigungen (Toasts)
- R�ckmeldungen des Systems M�SSEN fl�ssig eingeblendet werden.
- Die Art der Meldung (Erfolg, Warnung, Fehler) MUSS visuell sofort unterscheidbar sein.

### FR-003: Subtile Status-Anzeige
- Der Systemstatus MUSS pr�sent, aber nicht st�rend sein.
...
- **SC-003**: Die Anwendung f�hlt sich "fest" und "wertig" an.


---


## � Dokument: specify.md (Quelle: 061-Standard-pipeline)

---
id: SPEC-061
title: Standard Validation Pipeline (PVP)
tags: [Validation, Security, Integrity, SDD, Core-Grade]
status: cemented
version: 1.0.0
...
# SPEC-061: Standard Validation Pipeline (PVP)

## 1. Zielsetzung
Die Standard Validation Pipeline (PVP) ist das "Core Grade" Sicherheitsnetz des Projekts. Sie stellt sicher, dass kein Agent (KI oder Mensch) die obersten Mandate des Projekts unbemerkt verletzt. Sie automatisiert die Einhaltung der Architektur-Integrit�t und eliminiert Agenten-Halluzinationen durch SSoT-basierte Kataloge.
...
### 2.2 Automated Mandate Enforcement (Core Check)
Die Pipeline muss nach jeder �nderung folgende Pr�fungen automatisiert durchf�hren:

#### A. Security Check (MANDATE-INJ)
- **Regel**: `innerHTML` Zuweisungen sind verboten.
- **Toleranz**: 0 Verst��e erlaubt in `js/core/` und `js/ui/`.
...
- **Zweck**: Erhalt der Abh�ngigkeitsfreiheit und Browser-Nativit�t.

#### C. Integrity Check (MANDATE-PLN)
- **Regel**: Alle `<din-*>` Elemente im HTML m�ssen das Attribut `contenteditable="plaintext-only"` tragen.
- **Ausnahme**: Keine. Selbst der Body nutzt `plaintext-only` (Visualisierung via Mirror).

#### D. Visual Freeze Check (MANDATE-FREEZE)
- **Regel**: �nderungen an CSS-Layer `din.core` sind kritisch zu pr�fen.
...
- **Konformit�t**: 100% Abdeckung der Mandate in automatisierten Checks.


---


## � Dokument: specify.md (Quelle: 069-highlight)

---
id: SPEC-069
title: Native Highlight Editor � Paint-Time WYSIWYG
tags: [Core-grade, chrome-147, highlight-api, edit-context]
status: cemented
version: 1.0.0
traceability: [ADR-012, MANDATE-PLN]
source: Standard Audit 2026
---

# SPEC-069: Native Highlight Editor

## I. Zielsetzung (Core Grade)
Erm�glichung von WYSIWYG-Formatierungen (Fett, Kursiv), ohne die Plaintext-Integrit�t des DOM zu verletzen. Die Formatierung wird vollst�ndig von der Datenstruktur entkoppelt.

## II. Fachliche Anforderungen (WHAT)

### FR-001: Zero-Tag-Integrit�t
...
Formatierungen werden �ber Start- und End-Indizes (Offsets) definiert. 
Beispiel: `Text: "Hallo Welt"`, `Format: {type: 'bold', start: 0, end: 5}` -> "Hallo" erscheint fett.

### FR-003: Persistenz der Format-Matrix
Die Highlighting-Koordinaten m�ssen im Dokument-State gespeichert werden, um beim Neuladen wiederhergestellt zu werden.

### FR-004: Native Input-Souver�nit�t
Die Eingabe erfolgt �ber die **EditContext API**. Das DOM-Element dient nur als Projektionsfl�che f�r die Highlighting-Engine.

## III. Akzeptanzkriterien
1. `Strg+B` markiert den selektierten Text visuell fett, f�gt aber KEINE `<b>` oder `<strong>` Tags in das DOM ein.
2. Ein Export des Briefes als JSON enth�lt das Feld `body` als sauberen String ohne Steuerzeichen.
3. Die Highlighting-Ebene bleibt bei Scroll-Bewegungen und Fenster-Resizing mathematisch deckungsgleich mit dem Text.
4. Das Kopieren von formatiertem Text aus dem Editor resultiert in sauberem Plaintext in der Zwischenablage (Integrit�ts-Schutz).

## IV. Definition of Done
- [ ] Keine `<b>`, `<i>` oder `<span>` Tags im `<din-body>`.
- [ ] CSS `::highlight()` steuert die gesamte Optik.
- [ ] EditContext f�ngt alle OS-Eingaben ab.


---


## � Dokument: specify.md (Quelle: 066-markdown-ghosting)

---
id: SPEC-066
title: Markdown-Shredder V2 � Zero-Width Ghosting
tags: [Core-grade, Standard-2026, wysiwyg, integrity, css]
status: draft
version: 1.0.0
traceability: [ADR-008, ADR-011, MANDATE-FREEZE]
source: Standard Architecture Review 2026
---

# SPEC-066: Markdown-Shredder V2 (Zero-Width Ghosting)

## I. Zielsetzung (Core Grade)
Gew�hrleistung einer 100%igen �bereinstimmung der Zeilenumbr�che zwischen dem Editor (Ghost-Mirror) und dem fertigen Dokument (Print/PDF). Dies wird durch die Erhaltung aller Markdown-Steuerzeichen im Mirror erreicht, wobei diese f�r die Layout-Engine "gewichtslos" (Breite = 0) gemacht werden.

## II. Fachliche Anforderungen (WHAT)

### FR-001: Non-Destructive Transformation
Der Parser darf Steuerzeichen (*, **, ~~, _, >) nicht l�schen. Er muss sie in ein sch�tzendes Element (`<span class="md-marker">`) einwickeln, das die visuelle Formatierung (z.B. `<strong>`) umschlie�t oder flankiert.

### FR-002: Zero-Width Rendering
Die Marker-Elemente m�ssen f�r den Browser-Line-Breaker unsichtbar sein.
- Breite: 0px (zwingend)
- �berlauf: sichtbar (f�r menschliche Lesbarkeit)
- Interaktion: `pointer-events: none` und `user-select: none`

### FR-003: Kerning-Pr�zision
Um optisches "Kleben" von Markern an W�rtern zu verhindern, muss ein negativer Margin-Korrektor angewendet werden, der die physische Zeichenbreite im Editor neutralisiert.

### FR-004: EditContext-Offset-Parit�t
Die Anzahl der logischen Zeichen im `EditContext` muss exakt mit der Anzahl der sichtbaren (inkl. Ghosting) Zeichen im Mirror korrespondieren. HTML-Tags werden hierbei als Metadaten behandelt und d�rfen die Selektions-Logik nicht verschieben.

### FR-005: EditContext Input-Capture
Der Input-Ghost (EditContext) muss alle OS-Eingabemethoden (Virtual Keyboard, Handschrift, Spracheingabe) abfangen, ohne dass ein physisches DOM-Element f�r die Datenhaltung existiert. Das DOM dient nur noch als 'Viewport' f�r den Mirror.

### FR-006: Atomic Character Bounds
F�r die exakte Toolbar-Positionierung muss der EditContext die 'Character Bounds' des Mirrors kennen, damit das Anchor Positioning mathematisch korrekt auf den gerenderten Text (nicht auf den Plaintext) zeigt.
...
2. Die `Sanitizer API` l�sst die `.md-marker`-Spans ohne Datenverlust passieren.
3. EditContext Events (`textupdate`) triggern die Mirror-Synchronisation verz�gerungsfrei (< 16ms).
4. `contenteditable="plaintext-only"` wird durch das EditContext-Objekt als prim�re Eingabemethode abgel�st.

## V. RegEx Priority Cascade (Cascading Priority Parsing)
Um �berlappungen zu vermeiden, muss die Transformation in folgender Reihenfolge erfolgen:
...
## VI. Mathematischer Offset-Beweis (WYSIWYG-Parit�t)
...
- **Ergebnis**: Parit�t = 100%. Der Cursor-Offset im `EditContext` entspricht exakt der Zeichenposition im Mirror, da der Browser HTML-Tags bei der `textContent`-L�nge ignoriert.

## VII. Erweitertes Sanitizer-Gatekeeping
`Standard_SANITIZER_CONFIG` muss `span` und die spezifischen `md-*` Klassen erlauben:
~~~javascript
const Standard_SANITIZER_CONFIG = {
...
## � Dokument: specify.md (Quelle: 057-dom-first-integrity)

---
id: SPEC-057
title: DOM-First Integrity & Anchor Positioning
tags: [chrome-147, anchor-positioning, zero-js, Core-grade]
status: active
version: 1.0.0
...
## 1. Zielsetzung
JavaScript soll vollst�ndig von der Live-Synchronisation der Benutzereingaben entbunden werden. Das Dokument im Browser-Fenster (DOM) ist die einzige Quelle der Wahrheit w�hrend der Bearbeitung.
...
- Die Toolbar f�r Formatierungen MUSS sich physisch an dem Feld ausrichten, das gerade bearbeitet wird.
- Diese Ausrichtung MUSS fl�ssig und ohne JavaScript-Berechnungen erfolgen.

### FR-003: Unl�schbare Feld-Identit�ten
- Strukturelle Elemente des Briefes (wie das Wort "Datum" im Infoblock) M�SSEN f�r den Nutzer unl�schbar sein.
- Diese Elemente d�rfen nicht Teil des editierbaren Textes sein.
...
- **SC-003**: Ein "Alles l�schen" im Feld darf die Beschriftung (z.B. "Datum") nicht entfernen.


---


## � Dokument: specify.md (Quelle: 007-cma)

---
id: SPEC-007
title: Central Measurement Authority (CMA)
tags: [cma, din-5008, ssot, Core-grade]
status: cemented
weight: 100
criticality: CRITICAL
created: 2026-03-20
...
### Datentabelle: Kanonische DIN 5008-Masse (Core Grade)
...
## � Dokument: specify.md (Quelle: 002-salutation-engine)
...
## � Dokument: specify.md (Quelle: 029-page-breaks)

---
id: SPEC-029
title: Multi-Page Pagination & Hybrid Height
tags: [print, pagination, Core-grade]
status: cemented
version: 2.0.0
...
## 1. Zielsetzung
Das System muss in der Lage sein, Briefe zu verarbeiten, die l�nger als eine DIN-A4 Seite sind. W�hrend auf dem Bildschirm die Immersion eines einzelnen Blattes gewahrt bleibt, muss der Ausdruck (PDF) den Textfluss �ber beliebig viele Seiten erm�glichen.

## 2. Fachliche Anforderungen (WHAT)

### FR-001: Hybrid-Height Mode
- **Screen-Mode**: Das Papier hat eine fixe H�he von 297mm. Text, der dar�ber hinausgeht, wird visuell markiert (Overflow-W�chter), aber nicht gescrollt.
- **Print-Mode**: Das Papier wechselt auf eine variable H�he (`height: auto`). Der Text flie�t nat�rlich auf Folgeseiten.

### FR-002: Typografische Integrit�t auf Folgeseiten
- Seitenumbr�che M�SSEN so gesteuert werden, dass keine "Hurenkinder" oder "Schusterjungen" entstehen (mind. 3 Zeilen pro Absatz auf einer Seite).
- Tabellen oder Listen d�rfen nicht unkontrolliert zerrissen werden.

### FR-003: Folgeseiten-Header
- (Zuk�nftig): Folgeseiten sollten eine Seitenzahl ("Seite 2") enthalten.

## 3. Erfolgskriterien
- **SC-001**: Ein Brief mit 5000 W�rtern wird im Druck auf ca. 10 Seiten aufgeteilt.
- **SC-002**: Der Briefkopf (Absender, Empf�nger) erscheint nur auf der ersten Seite.
- **SC-003**: Die Textsch�rfe bleibt vektorbasiert.


---


## � Dokument: specify.md (Quelle: 056-environment-integrity)
...
## 1. Problemstellung
Klassische Web-Apps nutzen JavaScript, um UI-Zust�nde (z.B. Layout-Wechsel) zu verwalten. Dies f�hrt zu Redundanz, Latenz und "Pixel-Shock". Ziel ist die vollst�ndige Delegation der UI-Logik an die Browser-Engine.

## 2. Fachliche Anforderungen (WHAT)

### FR-001: Autonomes State-Management
- Das DOM MUSS der einzige Zustandshalter (SSoT) f�r UI-Optionen sein.
- Die Auswahl eines Layouts oder das Einblenden von Hilfsmitteln MUSS rein �ber native HTML-Formular-Elemente (Radio/Checkbox) erfolgen.

### FR-002: Implizite Visuelle R�ckmeldung
- Der Browser MUSS Zustands�nderungen eigenst�ndig animieren. 
- Es DARF KEINE explizite JavaScript-Anweisung f�r Animationen oder �berg�nge n�tig sein.

### FR-003: Isolierte Daten-Integrit�t (Data-IO)
- JavaScript darf NUR f�r die Verarbeitung von Briefinhalten (IMR) genutzt werden.
- Export, Import und Zur�cksetzen sind funktionale Aktionen, die vom UI-Zustand entkoppelt sein m�ssen.

## 3. Erfolgskriterien
- **SC-001**: 100% Funktionalit�t der UI-Steuerung bei deaktiviertem JavaScript (sofern die Engine CSS-Variablen aufl�st).
...
## � Dokument: specify.md (Quelle: 051-content-integrity)

---
id: SPEC-051
title: Content Integrity & Ghost-Mirror System
tags: [integrity, markdown, ghost-mirror, Core-grade]
status: active
version: 1.0.0
...
## 1. Problemstellung
`contenteditable="true"` erlaubt Browsern, beim Einf�gen (Paste) unkontrolliertes HTML in das Dokument zu schreiben. Dies f�hrt zu "Datenvergiftung" (unsichtbare Styles, Skripte, Spans), die bei einem Export oder einer KI-Verarbeitung zu Fehlern f�hren. 

## 2. Fachliche Anforderungen (WHAT)

### FR-001: Plaintext-Only Doctrine (ADR-008)
- Jedes Eingabefeld (<din-*>) MUSS strukturell gegen HTML-Injection gesch�tzt sein.
- Der Browser DARF KEIN HTML in diese Felder schreiben, auch nicht beim Paste oder Drag-and-Drop.
- Die einzige Datenquelle f�r den Export MUSS `textContent` sein.

### FR-002: Markdown-Support im Briefk�rper
- Der Nutzer MUSS Formatierungen im Feld `<din-body>` mittels einfacher Markdown-Syntax vornehmen k�nnen:
  - `**fett**` -> <strong>
  - `*kursiv*` -> <em>
  - `~~durchgestrichen~~` -> <del>
  - `> Zitat` -> <blockquote>
...
- Der Mirror MUSS exakt �ber dem Eingabefeld liegen, damit der optische Eindruck eines formatierten Briefes erhalten bleibt.
...
### FR-004: Typografische Integrit�t (Core Grade)
- Das System MUSS sicherstellen, dass Abs�tze nicht durch unsch�ne Seitenumbr�che zerrissen werden.
- Mindestens 3 Zeilen eines Absatzes M�SSEN am Seitenende oder -anfang zusammengehalten werden (Widows/Orphans).
- URLs M�SSEN im Druckmodus voll ausgeschrieben hinter dem Link-Text erscheinen.
...
## � Dokument: 06_validation_anchor_plan.md (Quelle: plans)
...
| Gate | Status | Notiz |
|------|--------|-------|
| [MANDATE-INJ] | OK | Nur textContent f�r Fehlermeldungen. |
| [MANDATE-NAT] | OK | Nutzung nativer CSS Anchor Positioning API. |
| [MANDATE-PLN] | OK | Keine Beeintr�chtigung der plaintext-only Felder. |

## 1. CSS CORE ARCHITECTURE (din.core)
Wir definieren die Ankerpunkte f�r alle IMR-Felder.
~~~css
...
/* Nur anzeigen, wenn das fokussierte Element ung�ltig ist */
...
1. [ ] CSS-Anker in `css/din5008-paper.css` hinzuf�gen.
2. [ ] Popover-HTML in `index.html` einf�gen.
3. [ ] `UIController.js` erweitern f�r `--active-anchor` Property.
4. [ ] `logic.js` erweitern (Validierungstrigger f�r `data-invalid`).
...
╚══════════════════════════════════════════════════════════════╝
Quelle     : C:\Users\morit\Desktop\DIN-BriefNEO\docs       
Dateien    : 4 Textdateien
Zeichen    : 386089
Zeilen     : 8851
Abgeschlossen: 2026-03-27 15:41:20
Fehler     : 0
```
